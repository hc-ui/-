# -*- coding: utf-8 -*-
"""Cut A/B shots, overlay captions, mix VO + pad + cut SFX."""
...
def main() -> None:
    ...
    project_final = ROOT / f"00_最终成片_{NAME}.mp4"
    run(["ffmpeg", "-y", "-i", str(final), "-c", "copy", "-movflags", "+faststart", str(project_final)])
    deliver = Path("/workspace/成片") / f"02-{NAME}.mp4"
    deliver.parent.mkdir(parents=True, exist_ok=True)
    run(["ffmpeg", "-y", "-i", str(final), "-c", "copy", "-movflags", "+faststart", str(deliver)])
    print("FINAL", final, "dur", probe_dur(final))
    print("DELIVER", deliver, "dur", probe_dur(deliver))
