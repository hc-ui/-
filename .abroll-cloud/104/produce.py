...
- **路径依据**：共享记忆库「本机公共路径与强制约束」——新视频进 `AI视频项目/<NN_中文项目名>/`，成品名 `00_最终成片_*`。本云端无本机盘符，中转 `成片/{STAGED_NAME}`，工程成片 `00_最终成片_{NAME}.mp4`
- **成片中转**：`成片/{STAGED_NAME}`
...
路径依据：记忆库新片进 `AI视频项目/104_金句要能截图/`，成品 `00_最终成片_金句要能截图.mp4`；本云端中转 `成片/{STAGED_NAME}`。
不重做 00–103，不抢 100–103，不拷工厂成片。不写 C/D。
""",
        encoding="utf-8",
    )
