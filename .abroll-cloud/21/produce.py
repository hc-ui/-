# -*- coding: utf-8 -*-
"""Topic 21 / 选题 003：钟指十二，东西往上落。云端 A-roll + B-roll。"""
...
ROOT = Path("/workspace/.abroll-cloud/21")
...
def render_b_law(duration: float) -> list[Image.Image]:
    ...
    for i in range(n):
        ...
        tag_chip(d, t, "改一条定律")
        a0 = appear(t, 0.02)
        d.text((W // 2, 250 + int(lerp(20, 0, a0))), "重力翻一次", font=title_f, fill=mix(BG, WHITE, a0), anchor="mm")
        ...
        if a1 > 0.04:
            ...
            d.text((540, y + 80), "永远往下掉", font=card_f, fill=mix(CARD, YELLOW, a1), anchor="mm")
            d.text((540, y + 160), "天花板只是顶", font=sub_f, fill=mix(CARD, MUTED, a1), anchor="mm")
        ...
        if a2 > 0.04:
            ...
            d.text((540, y + 90), "每到十二翻向", font=title_f, fill=mix(CARD, MINT, a2), anchor="mm")
            d.text((540, y + 180), "上和下对调", font=sub_f, fill=mix(CARD, WHITE, a2), anchor="mm")
        ...
        if punch > 0.04:
            ...
            d.text((W // 2, y + 100), "十二小时一翻", font=title_f, fill=mix(BG, YELLOW, punch), anchor="mm")
        out.append(img)
    return out
...
def render_b_still(src: Path, duration: float, tag: str, title: str, punch: str) -> list[Image.Image]:
    ...
    still = ImageEnhance.Brightness(still).enhance(1.0)
    n = max(1, round(duration * FPS))
    out = []
    for i in range(n):
        t = i / FPS
...
def render_broll(data: dict) -> None:
    ...
    jobs = [
        ...
        ("broll/B-上落.mp4", lambda d: render_b_still(ROOT / "assets/t21a-clock-fall-up.png", d, "钩子", "钟指到十二", "东西往上落")),
        ("broll/B-双地面.mp4", lambda d: render_b_still(ROOT / "assets/t21b-ceiling-floor.png", d, "后果 01", "天花板", "也是地面")),
        ("broll/B-家具.mp4", lambda d: render_b_still(ROOT / "assets/t21c-paired-furniture.png", d, "后果 02", "家具成对", "上下各钉一份")),
        ("broll/B-双城.mp4", lambda d: render_b_still(ROOT / "assets/t21d-dual-city.png", d, "回报", "窗外整座城", "楼上楼下都有门")),
    ]
    for rel, fn in jobs:
        dur = durs.get(rel)
        if not dur:
...
def assemble(data: dict) -> Path:
    ...
    staged = Path("/workspace/成片") / f"21-{NAME}.mp4"
...
def write_cover() -> Path:
    ...
    still = Image.open(ROOT / "assets/t21a-clock-fall-up.png").convert("RGB")
    ...
    img = Image.alpha_composite(img.convert("RGBA"), shade).convert("RGB")
    d = ImageDraw.Draw(img)
    d.text((540, 260), "钟指到十二", font=font(78), fill=WHITE, anchor="mm")
    d.text((540, 380), "东西往上落", font=font(64), fill=YELLOW, anchor="mm")
    d.text((540, 500), "重力每十二小时翻一次", font=font(34), fill=MINT, anchor="mm")
...
def main() -> None:
    for need in (
        ...
        ROOT / "assets/t21a-clock-fall-up.png",
        ROOT / "assets/t21b-ceiling-floor.png",
        ROOT / "assets/t21c-paired-furniture.png",
        ROOT / "assets/t21d-dual-city.png",
    ):
        if not need.exists():
