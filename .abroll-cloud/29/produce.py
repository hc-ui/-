#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""成片 29：故事没锁不准出提示词。云端 A-roll + B-roll，不是短剧。"""
...
def render_b_lock(duration: float) -> list[Image.Image]:
    ...
    for i in range(n):
        ...
        tag(d, t, "对照 顺序")
        a0 = appear(t, 0.02)
        d.text((W // 2, 250 + int(lerp(18, 0, a0))), "先锁故事", font=title_f, fill=mix(BG, WHITE, a0), anchor="mm")

        a1 = appear(t, 0.18)
        y = 360 + int(lerp(24, 0, a1))
        rounded(d, (90, y, 990, y + 280), 32, mix(BG, CARD, a1))
        d.text((160, y + 90), "先堆提示词", font=card_f, fill=mix(CARD, YELLOW, a1), anchor="lm")
        d.text((160, y + 180), "故事还空", font=sub_f, fill=mix(CARD, MUTED, a1), anchor="lm")
        strike_text(d, 160, y + 90, "先堆提示词", card_f, t, 0.58)

        a2 = appear(t, 0.88)
        y2 = 690 + int(lerp(24, 0, a2))
        rounded(d, (90, y2, 990, y2 + 280), 32, mix(BG, (22, 40, 36), a2))
        d.text((160, y2 + 90), "故事加镜头表", font=card_f, fill=mix(CARD, MINT, a2), anchor="lm")
        d.text((160, y2 + 180), "再写提示词", font=sub_f, fill=mix(CARD, WHITE, a2), anchor="lm")
        ...
        if punch > 0.04:
            ...
            d.text((W // 2, y3 + 100), "没锁别出词", font=title_f, fill=mix(BG, YELLOW, punch), anchor="mm")
        out.append(img)
    return out
...
def render_b_one(duration: float) -> list[Image.Image]:
    ...
    for i in range(n):
        ...
        tag(d, t, "对照 出片")
        a0 = appear(t, 0.02)
        d.text((W // 2, 250 + int(lerp(18, 0, a0))), "一次一条", font=title_f, fill=mix(BG, WHITE, a0), anchor="mm")

        a1 = appear(t, 0.18)
        y = 360 + int(lerp(24, 0, a1))
        rounded(d, (90, y, 990, y + 280), 32, mix(BG, CARD, a1))
        d.text((160, y + 90), "一堆一起渲", font=card_f, fill=mix(CARD, YELLOW, a1), anchor="lm")
        d.text((160, y + 180), "碰运气", font=sub_f, fill=mix(CARD, MUTED, a1), anchor="lm")
        strike_text(d, 160, y + 90, "一堆一起渲", card_f, t, 0.58)

        a2 = appear(t, 0.88)
        y2 = 690 + int(lerp(24, 0, a2))
        rounded(d, (90, y2, 990, y2 + 280), 32, mix(BG, (22, 40, 36), a2))
        d.text((160, y2 + 90), "一条成片", font=card_f, fill=mix(CARD, MINT, a2), anchor="lm")
        d.text((160, y2 + 180), "过了再下一条", font=sub_f, fill=mix(CARD, WHITE, a2), anchor="lm")
        check_badge(d, 850, y2 + 140, appear(t, 1.08, 0.22))
        ...
        if punch > 0.04:
            y3 = 1060 + int(lerp(22, 0, punch))
            rounded(d, (140, y3, 940, y3 + 200), 28, mix(BG, (42, 28, 18), punch))
            d.text((W // 2, y3 + 100), "别批量", font=title_f, fill=mix(BG, YELLOW, punch), anchor="mm")
        out.append(img)
    return out
...
def build_timeline(cues: list[tuple[float, float, str]], duration: float) -> dict:
    ...
    shots = [
        ...
        {"id": "S02", "kind": "B", "start": round(b1, 3), "end": round(b2, 3), "src": "broll/B-先锁故事.mp4", "line": p2[2], "broll": "lock_first"},
        {"id": "S03", "kind": "B", "start": round(b2, 3), "end": round(p3[1], 3), "src": "broll/B-一次一条.mp4", "line": p3[2], "broll": "one_cut"},
...
def make_cover() -> Path:
    ...
    d.text((W // 2, 160), "故事没锁", font=font(58), fill=WHITE, anchor="mm")
    d.text((W // 2, 250), "不准出提示词", font=font(58), fill=YELLOW, anchor="mm")
...
def write_docs(duration: float, staged: Path, data: dict) -> None:
    ...
    status = {
        "schema_version": 1,
        "project_name": "29_故事没锁不准出提示词",
        "video_type": "普通短视频",
        "episode": 29,
        "title": NAME,
        "source_note": "工作线-开源小工具与短片.md §短片创作纪律",
        "unused_of": "涨星靠外发 / 十二节气先锁十二张首帧",
        "voice": VOICE,
        "duration": round(duration, 3),
        "size": [W, H],
        "fps": FPS,
...
def main() -> None:
    ...
    print("render B-先锁故事", d1)
    frames_to_mp4(render_b_lock(d1 + 0.12), ROOT / "broll" / "B-先锁故事.mp4")
    print("render B-一次一条", d2)
    frames_to_mp4(render_b_one(d2 + 0.12), ROOT / "broll" / "B-一次一条.mp4")
