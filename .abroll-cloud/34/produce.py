#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""成片 34：口播结尾别报时长。云端 A-roll + B-roll，不是短剧。

选题 topics-batch2.md #34；工厂 350_口播结尾别报时长。
草稿只写 .abroll-cloud/34/，成品中转 成片/34-口播别报时长.mp4。
...
def render_b_wash(duration: float) -> list[Image.Image]:
    ...
    for i in range(n):
        ...
        tag(d, t, "错 · 报时长")
        a0 = appear(t, 0.02)
        d.text((W // 2, 236 + int(lerp(18, 0, a0))), "片尾盖住指令", font=title_f, fill=mix(BG, WHITE, a0), anchor="mm")

        a1 = appear(t, 0.16)
        y = 360 + int(lerp(24, 0, a1))
        rounded(d, (90, y, 990, y + 320), 32, mix(BG, CARD, a1))
        d.text((W // 2, y + 100), "去改结尾", font=card_f, fill=mix(CARD, MINT, a1), anchor="mm")
        d.text((W // 2, y + 200), "行动句刚落地", font=sub_f, fill=mix(CARD, MUTED, a1), anchor="mm")

        cover = appear(t, 0.78, 0.34)
        if cover > 0.04:
            drop = int(lerp(-40, 0, cover))
            y2 = y + 40 + drop
            rounded(d, (150, y2, 930, y2 + 240), 28, mix(BG, (56, 28, 24), cover))
            d.text((W // 2, y2 + 80), "本片 12 秒", font=stamp_f, fill=mix(CARD, YELLOW, cover), anchor="mm")
            d.text((W // 2, y2 + 168), "倒数盖过指令", font=sub_f, fill=mix(CARD, RED, cover), anchor="mm")

        punch = appear(t, 1.42, 0.26)
        if punch > 0.04:
            y3 = 1060 + int(lerp(22, 0, punch))
            rounded(d, (140, y3, 940, y3 + 200), 28, mix(BG, (42, 28, 18), punch))
            d.text((W // 2, y3 + 100), "动作指令被冲掉", font=title_f, fill=mix(BG, YELLOW, punch), anchor="mm")
        out.append(img)
    return out


def render_b_cut(duration: float) -> list[Image.Image]:
    """对照：结尾只留一个动词；脚本里时长那行被删。"""
    n = max(1, round(duration * FPS))
    title_f = font(52)
    card_f = font(44)
    sub_f = font(32)
    ...
    for i in range(n):
        ...
        tag(d, t, "改法 · 删时长")
        a0 = appear(t, 0.02)
        d.text((W // 2, 236 + int(lerp(18, 0, a0))), "结尾只留动词", font=title_f, fill=mix(BG, WHITE, a0), anchor="mm")

        a1 = appear(t, 0.16)
        y = 360 + int(lerp(24, 0, a1))
        rounded(d, (90, y, 990, y + 280), 32, mix(BG, (22, 40, 36), a1))
        d.text((160, y + 90), "去改结尾", font=card_f, fill=mix(CARD, MINT, a1), anchor="lm")
        d.text((160, y + 180), "收束只留一个动作", font=sub_f, fill=mix(CARD, WHITE, a1), anchor="lm")
        check_badge(d, 850, y + 140, appear(t, 0.48, 0.22))

        a2 = appear(t, 0.72)
        y2 = 690 + int(lerp(24, 0, a2))
        box = (90, y2, 990, y2 + 280)
        rounded(d, box, 32, mix(BG, CARD, a2))
        d.text((160, y2 + 90), "本片 12 秒", font=card_f, fill=mix(CARD, YELLOW, a2), anchor="lm")
        d.text((160, y2 + 180), "脚本里时长那行", font=sub_f, fill=mix(CARD, MUTED, a2), anchor="lm")
        strike_box(d, (140, y2 + 60, 720, y2 + 120), appear(t, 1.05, 0.28), mix(CARD, RED, appear(t, 1.05, 0.28)))

        punch = appear(t, 1.42, 0.26)
        if punch > 0.04:
            y3 = 1060 + int(lerp(22, 0, punch))
            rounded(d, (140, y3, 940, y3 + 200), 28, mix(BG, (18, 42, 36), punch))
            d.text((W // 2, y3 + 100), "把时长删掉", font=title_f, fill=mix(BG, MINT, punch), anchor="mm")
        out.append(img)
    return out
...
def build_timeline(cues: list[tuple[float, float, str]], duration: float) -> dict:
    ...
    shots = [
        ...
        {"id": "S02", "kind": "B", "start": round(b1, 3), "end": round(p2[1], 3), "src": "broll/B-时长冲掉.mp4", "line": p2[2], "broll": "wash"},
        {"id": "S03", "kind": "A", "start": round(p2[1], 3), "end": round(b2, 3), "src": "assets/V-指向.mp4", "line": p3[2]},
        {"id": "S04", "kind": "B", "start": round(b2, 3), "end": round(duration, 3), "src": "broll/B-时长删掉.mp4", "line": p4[2], "broll": "cut"},
    ]
...
def make_cover() -> Path:
    ...
    d.text((W // 2, 160), "口播结尾", font=font(58), fill=WHITE, anchor="mm")
    d.text((W // 2, 250), "别报时长", font=font(58), fill=YELLOW, anchor="mm")
...
def write_docs(duration: float, staged: Path, data: dict) -> None:
    ...
    status = {
        "schema_version": 1,
        "project_name": "34_口播结尾别报时长",
        "video_type": "普通短视频",
        "episode": 34,
        "title": NAME,
        "slug": "350_口播结尾别报时长",
        "source_note": "topics-batch2.md #34 / 工厂 350_口播结尾别报时长 voiceover.txt",
        "unused_of": "口播别念链接 / 列表项先给结果 / 值班手机没有备机号",
        "voice": VOICE,
        "duration": round(duration, 3),
        "size": [W, H],
        "fps": FPS,
        ...
        "windows_paths": False,
        "drive_upload": False,
        "not": "drama-pipeline / 仙侠连载 / 别念链接 / 值班备机",
        "updated_at": now,
    }
...
def patch_delivery(duration: float, spec: str) -> None:
    ...
    row = f"| `{rel}` | {spec} | {duration:.2f}s | topic 34 工厂 350 口播结尾别报时长 |"
    lines = []
    replaced = False
    for raw in text.splitlines():
...
def main() -> None:
    ...
    frames_to_mp4(render_b_wash(d1 + 0.12), ROOT / "broll" / "B-时长冲掉.mp4")
    print("render B-时长删掉", d2)
    frames_to_mp4(render_b_cut(d2 + 0.12), ROOT / "broll" / "B-时长删掉.mp4")
