#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""成片 34（改拍）：口播别念屏幕上的字。云端 A-roll + B-roll，不是短剧。

选题 topics-batch2.md #24；工厂 311_口播别念屏幕上的字。
草稿只写 .abroll-cloud/34b/，成品中转 成片/34-口播别念屏幕上的字.mp4。
旧 34-口播别报时长.mp4 保留（与成片 24 同题）。
禁止盘符路径，禁止 Drive 上传，不走 drama-pipeline。
"""
...
def render_b_crash(duration: float) -> list[Image.Image]:
    """字幕和口播同一句叠在一起：耳眼撞车。"""
    ...
    for i in range(n):
        ...
        tag(d, t, "错 · 念屏")
        a0 = appear(t, 0.02)
        d.text((W // 2, 236 + int(lerp(18, 0, a0))), "耳眼撞车", font=title_f, fill=mix(BG, WHITE, a0), anchor="mm")

        a1 = appear(t, 0.16)
        y = 340 + int(lerp(24, 0, a1))
        rounded(d, (72, y, 508, y + 420), 28, mix(BG, CARD, a1))
        d.text((290, y + 78), "眼 · 屏幕", font=sub_f, fill=mix(CARD, MUTED, a1), anchor="mm")
        d.text((290, y + 190), "去改结尾", font=card_f, fill=mix(CARD, YELLOW, a1), anchor="mm")
        d.text((290, y + 300), "字已经上屏", font=sub_f, fill=mix(CARD, MUTED, a1), anchor="mm")

        a2 = appear(t, 0.28)
        rounded(d, (572, y, 1008, y + 420), 28, mix(BG, CARD, a2))
        d.text((790, y + 78), "耳 · 嘴里", font=sub_f, fill=mix(CARD, MUTED, a2), anchor="mm")
        d.text((790, y + 190), "去改结尾", font=card_f, fill=mix(CARD, RED, a2), anchor="mm")
        d.text((790, y + 300), "又念同一句", font=sub_f, fill=mix(CARD, MUTED, a2), anchor="mm")

        crash = appear(t, 0.92, 0.28)
        if crash > 0.04:
            cx = W // 2
            cy = y + 210
            d.ellipse((cx - 54, cy - 54, cx + 54, cy + 54), fill=mix(CARD, (56, 28, 24), crash))
            d.text((cx, cy), "撞", font=title_f, fill=mix(CARD, YELLOW, crash), anchor="mm")

        punch = appear(t, 1.42, 0.26)
        if punch > 0.04:
            y3 = 1060 + int(lerp(22, 0, punch))
            rounded(d, (140, y3, 940, y3 + 200), 28, mix(BG, (42, 28, 18), punch))
            d.text((W // 2, y3 + 100), "两头都听不清", font=title_f, fill=mix(BG, YELLOW, punch), anchor="mm")
        out.append(img)
    return out


def render_b_cut(duration: float) -> list[Image.Image]:
    """左屏放词、右嘴说另一句；脚本里念屏那行划掉。"""
    n = max(1, round(duration * FPS))
    title_f = font(52)
    card_f = font(40)
    sub_f = font(30)
    ...
    for i in range(n):
        ...
        tag(d, t, "改法 · 删念屏")
        a0 = appear(t, 0.02)
        d.text((W // 2, 236 + int(lerp(18, 0, a0))), "画面放词", font=title_f, fill=mix(BG, WHITE, a0), anchor="mm")

        a1 = appear(t, 0.16)
        y = 340 + int(lerp(24, 0, a1))
        rounded(d, (72, y, 508, y + 300), 28, mix(BG, (22, 40, 36), a1))
        d.text((290, y + 80), "屏幕放大", font=sub_f, fill=mix(CARD, MUTED, a1), anchor="mm")
        d.text((290, y + 180), "去改结尾", font=card_f, fill=mix(CARD, MINT, a1), anchor="mm")

        a2 = appear(t, 0.28)
        rounded(d, (572, y, 1008, y + 300), 28, mix(BG, CARD, a2))
        d.text((790, y + 80), "嘴里说话", font=sub_f, fill=mix(CARD, MUTED, a2), anchor="mm")
        d.text((790, y + 180), "下一步怎么做", font=card_f, fill=mix(CARD, WHITE, a2), anchor="mm")
        check_badge(d, 930, y + 70, appear(t, 0.55, 0.22))

        a3 = appear(t, 0.72)
        y2 = 690 + int(lerp(24, 0, a3))
        rounded(d, (90, y2, 990, y2 + 260), 32, mix(BG, CARD, a3))
        d.text((160, y2 + 80), "念屏：去改结尾", font=card_f, fill=mix(CARD, YELLOW, a3), anchor="lm")
        d.text((160, y2 + 176), "脚本里念屏那行", font=sub_f, fill=mix(CARD, MUTED, a3), anchor="lm")
        strike_box(d, (140, y2 + 52, 820, y2 + 112), appear(t, 1.05, 0.28), mix(CARD, RED, appear(t, 1.05, 0.28)))

        punch = appear(t, 1.42, 0.26)
        if punch > 0.04:
            y3 = 1060 + int(lerp(22, 0, punch))
            rounded(d, (140, y3, 940, y3 + 200), 28, mix(BG, (18, 42, 36), punch))
            d.text((W // 2, y3 + 100), "把念屏那句删掉", font=title_f, fill=mix(BG, MINT, punch), anchor="mm")
        out.append(img)
    return out
...
def build_timeline(cues: list[tuple[float, float, str]], duration: float) -> dict:
    ...
    shots = [
        ...
        {"id": "S02", "kind": "B", "start": round(b1, 3), "end": round(p2[1], 3), "src": "broll/B-耳眼撞车.mp4", "line": p2[2], "broll": "crash"},
        {"id": "S03", "kind": "A", "start": round(p2[1], 3), "end": round(b2, 3), "src": "assets/V-指向.mp4", "line": p3[2]},
        {"id": "S04", "kind": "B", "start": round(b2, 3), "end": round(duration, 3), "src": "broll/B-念屏删掉.mp4", "line": p4[2], "broll": "cut"},
    ]
    ...
    a_caps = [
        ...
        {"start": round(p3[0], 3), "end": round(min(p3[1], b2), 3), "lines": split_caption(p3[2])},
    ]
    ...
    eyebrows = [
        ...
        {"start": round(p3[0], 3), "end": round(min(p3[1], b2), 3), "text": "A-ROLL / 03"},
    ]
    data = {
...
def make_cover() -> Path:
    ...
    d.text((W // 2, 160), "口播别念", font=font(58), fill=WHITE, anchor="mm")
    d.text((W // 2, 250), "屏幕上的字", font=font(58), fill=YELLOW, anchor="mm")
...
def write_docs(duration: float, staged: Path, data: dict) -> None:
    ...
    status = {
        "schema_version": 1,
        "project_name": "34_口播别念屏幕上的字",
        "video_type": "普通短视频",
        "episode": 34,
        "title": NAME,
        "slug": "311_口播别念屏幕上的字",
        "source_note": "topics-batch2.md #24 / 工厂 311_口播别念屏幕上的字 voiceover.txt",
        "unused_of": "口播别念链接 / 口播结尾别报时长",
        "voice": VOICE,
        "duration": round(duration, 3),
        "size": [W, H],
        "fps": FPS,
        ...
        "windows_paths": False,
        "drive_upload": False,
        "not": "drama-pipeline / 仙侠连载 / 别念链接 / 别报时长",
        "updated_at": now,
    }
...
def patch_delivery(duration: float, spec: str) -> None:
    path = Path("/workspace/.abroll-cloud/DELIVERY.md")
    if not path.exists():
        return
    text = path.read_text(encoding="utf-8")
    rel = f"成片/{STAGED_NAME}"
    row = f"| `{rel}` | {spec} | {duration:.2f}s | topic 24 工厂 311 口播别念屏幕上的字 |"
    lines = []
    replaced = False
    for raw in text.splitlines():
        if f"`{rel}`" in raw and raw.strip().startswith("|"):
            lines.append(row)
            replaced = True
            continue
        lines.append(raw)
    ...
    if "## 本轮 30–37" not in text:
        block = [
            ...
            "| 号 | 文件 | 状态 |",
            "|----|------|------|",
            f"| 34 | `{STAGED_NAME}` | 已核验 {duration:.1f}s |",
        ]
        if not text.endswith("\n"):
            text += "\n"
        text += "\n".join(block) + "\n"
    else:
        old = re.compile(r"^\| 34 \| .+ \|", re.M)
        new = f"| 34 | `{STAGED_NAME}` | 已核验 {duration:.1f}s |"
        if old.search(text):
...
def main() -> None:
    ...
    print("render B-耳眼撞车", d1)
    frames_to_mp4(render_b_crash(d1 + 0.12), ROOT / "broll" / "B-耳眼撞车.mp4")
    print("render B-念屏删掉", d2)
    frames_to_mp4(render_b_cut(d2 + 0.12), ROOT / "broll" / "B-念屏删掉.mp4")

    make_cover()
    staged = assemble(data)
    dur = probe_dur(staged)
    write_docs(dur, staged, data)
    spec = "1080×1920 h264+aac 44.1k stereo"
    patch_index(dur)
    patch_delivery(dur, spec)
