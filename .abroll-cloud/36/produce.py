#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""成片 36：别说下一期。工厂 329 口播工艺。云端 A-roll + B-roll，不是短剧。"""
...
def render_b_wait(duration: float) -> list[Image.Image]:
    ...
    for i in range(n):
        ...
        tag(d, t, "错 · 预告")
        a0 = appear(t, 0.02)
        d.text((W // 2, 236 + int(lerp(16, 0, a0))), "下期更精彩", font=font(58), fill=mix(BG, WHITE, a0), anchor="mm")

        a1 = appear(t, 0.16)
        if a1 > 0.04:
            y = 330 + int(lerp(22, 0, a1))
            rounded(d, (80, y, 1000, y + 340), 32, mix(BG, CARD, a1))
            d.text((160, y + 70), "这期完播", font=font(40), fill=mix(CARD, MUTED, a1), anchor="lm")
            drain = appear(t, 0.72, 0.45)
            fill_w = int(lerp(760, 150, drain))
            bar = (160, y + 140, 920, y + 210)
            rounded(d, bar, 18, mix(CARD, (40, 44, 52), a1))
            if fill_w > 8:
                rounded(d, (160, y + 140, 160 + fill_w, y + 210), 18, mix(CARD, MINT, a1))
            pct = int(lerp(92, 18, drain))
            d.text((W // 2, y + 175), f"{pct}%", font=font(36), fill=mix(CARD, INK if drain < 0.55 else WHITE, a1), anchor="mm")
            d.text((W // 2, y + 270), "预告一出，进度被冲掉", font=font(32), fill=mix(CARD, YELLOW, a1), anchor="mm")

        a2 = appear(t, 0.40)
        if a2 > 0.04:
            y = 720 + int(lerp(20, 0, a2))
            rounded(d, (80, y, 1000, y + 280), 32, mix(BG, CARD, a2))
            d.text((W // 2, y + 80), "这期还没听完", font=font(44), fill=mix(CARD, WHITE, a2), anchor="mm")
            d.text((W // 2, y + 180), "人已经在等下一期", font=font(40), fill=mix(CARD, RED, a2), anchor="mm")
            strike_line(d, (180, y + 40, 900, y + 120), appear(t, 1.05, 0.28), mix(CARD, RED, a2))

        punch = appear(t, 1.42, 0.24)
        if punch > 0.04:
            y = 1080 + int(lerp(22, 0, punch))
            rounded(d, (120, y, 960, y + 200), 28, mix(BG, (42, 24, 22), punch))
            d.text((W // 2, y + 100), "完播被冲掉", font=font(52), fill=mix(BG, YELLOW, punch), anchor="mm")
        out.append(img)
    return out
...
def render_b_cut(duration: float) -> list[Image.Image]:
    ...
    for i in range(n):
        ...
        tag(d, t, "对 · 这期")
        a0 = appear(t, 0.02)
        d.text((W // 2, 236 + int(lerp(16, 0, a0))), "收束落在这期", font=font(54), fill=mix(BG, WHITE, a0), anchor="mm")

        a1 = appear(t, 0.16)
        if a1 > 0.04:
            y = 330 + int(lerp(20, 0, a1))
            rounded(d, (80, y, 520, y + 420), 32, mix(BG, (22, 40, 36), a1))
            d.text((300, y + 90), "留", font=font(30), fill=mix(CARD, MUTED, a1), anchor="mm")
            d.text((300, y + 180), "这期动作", font=font(44), fill=mix(CARD, MINT, a1), anchor="mm")
            d.text((300, y + 280), "可做的一步", font=font(32), fill=mix(CARD, WHITE, a1), anchor="mm")
        ...
        if a2 > 0.04:
            ...
            d.text(((x0 + x1) / 2, y0 + 90), "撕", font=font(30), fill=mix(CARD, MUTED, a2), anchor="mm")
            d.text(((x0 + x1) / 2, y0 + 180), "下期见", font=font(44), fill=mix(CARD, YELLOW, a2), anchor="mm")
            d.text(((x0 + x1) / 2, y0 + 280), "预告整段", font=font(32), fill=mix(CARD, MUTED, a2), anchor="mm")
            if tear > 0.08:
        ...
        if punch > 0.04:
            ...
            d.text((W // 2, y + 100), "把预告删掉", font=font(52), fill=mix(BG, MINT, punch), anchor="mm")
        out.append(img)
    return out
...
def build_timeline(cues: list[tuple[float, float, str]], duration: float) -> dict:
    ...
    shots = [
        ...
        {"id": "S02", "kind": "B", "start": b1, "end": p2[1], "src": "broll/B-等下一期.mp4", "line": p2[2], "broll": "wait_next"},
        {"id": "S03", "kind": "A", "start": p2[1], "end": b2, "src": "assets/V-指向.mp4", "line": p3[2]},
        {"id": "S04", "kind": "B", "start": b2, "end": duration, "src": "broll/B-预告删掉.mp4", "line": p4[2], "broll": "cut_teaser"},
    ]
    ...
    data = {
        ...
        "a_caps": [
            ...
            {"start": round(p1[0], 3), "end": round(b1, 3), "lines": ["口播里别说下一期"]},
            {"start": round(p3[0], 3), "end": round(b2, 3), "lines": ["收束落在这期动作"]},
        ],
        "shutters": [
...
def make_cover() -> Path:
    ...
    d.text((W // 2, 160), "口播里", font=font(48), fill=WHITE, anchor="mm")
    d.text((W // 2, 250), "别说下一期", font=font(64), fill=YELLOW, anchor="mm")
...
def write_docs(duration: float, staged: Path, data: dict) -> None:
    ...
    status = {
        "schema_version": 1,
        "project_name": "36_别说下一期",
        "video_type": "普通短视频",
        "episode": 36,
        "title": NAME,
        "full_title": FULL_TITLE,
        "source_note": "topics-batch2.md #36 · 工厂 329_口播里别说下一期",
        "slug": "329_口播里别说下一期",
        "voice": VOICE,
        "duration": round(duration, 3),
        "size": [W, H],
...
def main() -> None:
    ...
    print("render B-等下一期", d1)
    frames_to_mp4(render_b_wait(d1 + 0.12), ROOT / "broll" / "B-等下一期.mp4")
    print("render B-预告删掉", d2)
    frames_to_mp4(render_b_cut(d2 + 0.12), ROOT / "broll" / "B-预告删掉.mp4")
