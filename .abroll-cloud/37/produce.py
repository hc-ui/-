#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""成片 37：组会撞课先摸惯例再书面报备。云端 A-roll + B-roll，不是短剧。"""
...
NAME = "组会撞课先摸惯例再书面报备"
STAGED_NAME = "37-组会撞课先摸惯例再书面报备.mp4"
...
def draw_calendar(draw, cx: int, cy: int, scale: float, a: float) -> None:
    if a <= 0.04:
        return
    w, h = int(200 * scale), int(240 * scale)
    x0, y0 = cx - w // 2, cy - h // 2
    rounded(draw, (x0, y0, x0 + w, y0 + h), 22, mix(BG, CARD, a))
    rounded(draw, (x0, y0, x0 + w, y0 + int(54 * scale)), 22, mix(CARD, YELLOW, a))
    draw.rectangle((x0, y0 + int(36 * scale), x0 + w, y0 + int(54 * scale)), fill=mix(CARD, YELLOW, a))
    fnt = font(max(18, int(22 * scale)))
    draw.text((cx, y0 + int(26 * scale)), "课表", font=fnt, fill=mix(YELLOW, INK, a), anchor="mm")
    pad = int(22 * scale)
    gap = int(18 * scale)
    cell = int(22 * scale)
    for r in range(4):
        for c in range(5):
            px = x0 + pad + c * (cell + gap // 2)
            py = y0 + int(78 * scale) + r * (cell + gap)
            hit = r == 1 and c == 2
            col = mix(CARD, RED if hit else MUTED, a)
            draw.rounded_rectangle((px, py, px + cell, py + cell), 5, fill=col)


def draw_alarm(draw, cx: int, cy: int, scale: float, a: float) -> None:
    if a <= 0.04:
        return
    r = int(96 * scale)
    draw.ellipse((cx - r, cy - r, cx + r, cy + r), fill=mix(BG, CARD, a), outline=mix(CARD, MINT, a), width=8)
    br = int(28 * scale)
    draw.ellipse((cx - r + 8, cy - r - 10, cx - r + 8 + br, cy - r - 10 + br), fill=mix(CARD, MINT, a))
    draw.ellipse((cx + r - 8 - br, cy - r - 10, cx + r - 8, cy - r - 10 + br), fill=mix(CARD, MINT, a))
    draw.ellipse((cx - 10, cy - 10, cx + 10, cy + 10), fill=mix(CARD, WHITE, a))
    draw.line([(cx, cy), (cx + int(42 * scale), cy + 8)], fill=mix(CARD, YELLOW, a), width=8)
    draw.line([(cx, cy), (cx - 6, cy - int(50 * scale))], fill=mix(CARD, WHITE, a), width=6)
    fnt = font(max(18, int(22 * scale)))
    draw.text((cx, cy + r + int(28 * scale)), "组会", font=fnt, fill=mix(BG, MINT, a), anchor="mm")
...
def render_b_steps(duration: float) -> list[Image.Image]:
    ...
    out = []
    rows = [
        (0.72, "1", "摸同门惯例", "私下问怎么请假", YELLOW),
        (1.12, "2", "书面报备", "培养方案刚性", MINT),
        (1.52, "3", "随假给选项", "PPT 加二选一", CREAM),
    ]
    for i in range(n):
        t = i / FPS
        img = new_bg()
        ...
        tag(d, t, "撞课三步")
        a0 = appear(t, 0.02)
        d.text((W // 2, 236 + int(lerp(18, 0, a0))), "先摸惯例", font=title_f, fill=mix(BG, WHITE, a0), anchor="mm")

        crash = min(1.0, t / 0.55)
        left = int(lerp(210, 330, ease_out(crash)))
        right = int(lerp(870, 750, ease_out(crash)))
        draw_calendar(d, left, 430, 1.0, appear(t, 0.04, 0.22))
        draw_alarm(d, right, 430, 1.0, appear(t, 0.08, 0.22))
        boom = appear(t, 0.50, 0.18)
        if boom > 0.04:
            d.ellipse((470, 360, 610, 500), outline=mix(BG, YELLOW, boom), width=int(8 * boom + 2))
            d.text((W // 2, 560), "对撞", font=sub_f, fill=mix(BG, YELLOW, boom), anchor="mm")

        for idx, (ts, num, head, body, color) in enumerate(rows):
            a = appear(t, ts, 0.24)
            if a < 0.04:
                continue
            y = 620 + idx * 196 + int(lerp(22, 0, a))
            rounded(d, (90, y, 990, y + 176), 28, mix(BG, CARD, a))
            d.ellipse((128, y + 44, 220, y + 136), fill=mix(CARD, color, a))
            d.text((174, y + 90), num, font=num_f, fill=mix(color, INK, a), anchor="mm")
        ...
        if punch > 0.04:
            y = 1230 + int(lerp(20, 0, punch))
            rounded(d, (160, y, 920, y + 150), 26, mix(BG, (42, 28, 18), punch))
            d.text((W // 2, y + 75), "按惯例走", font=title_f, fill=mix(BG, YELLOW, punch), anchor="mm")
        out.append(img)
    return out


def render_b_report(duration: float) -> list[Image.Image]:
    n = max(1, round(duration * FPS))
    title_f = font(56)
    card_f = font(40)
    sub_f = font(28)
    ...
    out = []
    rows = [
        (0.10, "课", "必修课", "考勤占比写清", YELLOW),
        (0.32, "附", "本周PPT", "随假先交上", MINT),
        (0.54, "甲", "先汇报", "报完再去上课", CREAM),
        (0.76, "乙", "课后补", "或下次组会补", MINT),
    ]
    for i in range(n):
        t = i / FPS
        img = new_bg()
        ...
        tag(d, t, "报备结构")
        a0 = appear(t, 0.02)
        d.text((W // 2, 236 + int(lerp(16, 0, a0))), "只写结构", font=title_f, fill=mix(BG, WHITE, a0), anchor="mm")

        for idx, (ts, num, head, body, color) in enumerate(rows):
            a = appear(t, ts, 0.22)
            if a < 0.04:
                continue
            y = 320 + idx * 168 + int(lerp(18, 0, a))
            rounded(d, (90, y, 990, y + 150), 26, mix(BG, CARD, a))
            d.rounded_rectangle((120, y + 40, 214, y + 112), 16, fill=mix(CARD, color, a))
            d.text((167, y + 76), num, font=num_f, fill=mix(color, INK, a), anchor="mm")
            d.text((244, y + 48), head, font=card_f, fill=mix(CARD, color, a), anchor="lm")
            d.text((244, y + 106), body, font=sub_f, fill=mix(CARD, WHITE, a), anchor="lm")

        bad = appear(t, 1.18, 0.22)
        if bad > 0.04:
            y = 1010 + int(lerp(16, 0, bad))
            rounded(d, (90, y, 990, y + 160), 26, mix(BG, CARD, bad))
            d.text((160, y + 80), "下周一定做完", font=card_f, fill=mix(CARD, MUTED, bad), anchor="lm")
            strike_text(d, 160, y + 80, "下周一定做完", card_f, t, 1.38)

        good = appear(t, 1.48, 0.22)
        if good > 0.04:
            y = 1190 + int(lerp(16, 0, good))
            rounded(d, (90, y, 990, y + 160), 26, mix(BG, (22, 40, 36), good))
            d.text((160, y + 80), "已完成动作", font=card_f, fill=mix(CARD, MINT, good), anchor="lm")
            check_badge(d, 860, y + 80, appear(t, 1.62, 0.20))

        punch = appear(t, 1.86, 0.22)
        if punch > 0.04:
            y = 1380 + int(lerp(16, 0, punch))
            rounded(d, (160, y, 920, y + 140), 26, mix(BG, (42, 28, 18), punch))
            d.text((W // 2, y + 70), "两个选项", font=title_f, fill=mix(BG, YELLOW, punch), anchor="mm")
        out.append(img)
    return out
...
def build_timeline(cues: list[tuple[float, float, str]], duration: float) -> dict:
    ...
    shots = [
        ...
        {"id": "S02", "kind": "B", "start": round(b1, 3), "end": round(b2, 3), "src": "broll/B-撞课三步.mp4", "line": p2[2], "broll": "three_steps"},
        {"id": "S03", "kind": "B", "start": round(b2, 3), "end": round(p3[1], 3), "src": "broll/B-报备结构.mp4", "line": p3[2], "broll": "report_structure"},
...
def make_cover() -> Path:
    ...
    d.text((W // 2, 160), "组会撞课", font=font(58), fill=WHITE, anchor="mm")
    d.text((W // 2, 250), "先摸惯例再报备", font=font(52), fill=YELLOW, anchor="mm")
...
def write_docs(duration: float, staged: Path, data: dict) -> None:
    ...
    status = {
        "schema_version": 1,
        "project_name": "37_组会撞课先摸惯例再书面报备",
        "video_type": "普通短视频",
        "episode": 37,
        "title": NAME,
        "source_note": "赵刚课题组备忘录·第三节 SOP；topics-batch2.md #37",
        "unused_of": "组会只报做完的动作（成片16主标题）/ 杂务别接报销采购（成片28）",
        "voice": VOICE,
        "duration": round(duration, 3),
        "size": [W, H],
        "fps": FPS,
...
def patch_delivery(duration: float) -> None:
    ...
    marker = "| 37 | — | 待收 |"
    row = f"| 37 | `{STAGED_NAME}` | 已核验 {duration:.1f}s |"
    if marker in text:
...
def patch_topics(duration: float) -> None:
    path = Path("/workspace/.abroll-cloud/topics-batch2.md")
    if not path.exists():
        return
    text = path.read_text(encoding="utf-8")
    old = "| 37 | 专硕 SOP | `37-组会撞课先摸惯例.mp4` | 未拍 |"
    new = f"| 37 | 专硕 SOP | `{STAGED_NAME}` | 已核验 {duration:.1f}s |"
    if old in text:
...
def main() -> None:
    ...
    print("render B-撞课三步", d1)
    frames_to_mp4(render_b_steps(d1 + 0.12), ROOT / "broll" / "B-撞课三步.mp4")
    print("render B-报备结构", d2)
    frames_to_mp4(render_b_report(d2 + 0.12), ROOT / "broll" / "B-报备结构.mp4")
