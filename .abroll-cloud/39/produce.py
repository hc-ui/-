#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""成片 39：推不出来的酷画面删掉。云端 A-roll + B-roll，不是短剧。"""
...
def render_b_law(duration: float) -> list[Image.Image]:
    ...
    out = []
    rows = [
        (0.42, "1", "后果一", "直接推出来", YELLOW),
        (0.68, "2", "后果二", "还是这条定律", MINT),
        (0.94, "3", "后果三", "放大当回报", CREAM),
    ]
    for i in range(n):
        t = i / FPS
        img = new_bg()
        ...
        tag(d, t, "结构 固定")
        a0 = appear(t, 0.02)
        d.text((W // 2, 220 + int(lerp(16, 0, a0))), "改一条定律", font=title_f, fill=mix(BG, WHITE, a0), anchor="mm")

        law = appear(t, 0.10, 0.24)
        if law > 0.04:
            rounded(d, (180, 280, 900, 430), 28, mix(BG, CARD, law))
            d.text((W // 2, 355), "只改这一条", font=card_f, fill=mix(CARD, YELLOW, law), anchor="mm")
        ...
        for idx, (ts, num, head, body, color) in enumerate(rows):
            ...
            if a < 0.04:
                continue
            x = 70 + idx * 340
            y = 620 + int(lerp(22, 0, a))
            if appear(t, 0.28, 0.20) > 0.2:
                d.line([(W // 2, 430), (x + 150, y)], fill=mix(BG, MUTED, a), width=6)
            rounded(d, (x, y, x + 300, y + 280), 26, mix(BG, CARD, a))
            d.ellipse((x + 110, y + 28, x + 190, y + 108), fill=mix(CARD, color, a))
            d.text((x + 150, y + 68), num, font=font(36), fill=mix(color, INK, a), anchor="mm")
            d.text((x + 150, y + 160), head, font=card_f, fill=mix(CARD, color, a), anchor="mm")
            d.text((x + 150, y + 220), body, font=sub_f, fill=mix(CARD, WHITE, a), anchor="mm")

        punch = appear(t, 1.42, 0.22)
        if punch > 0.04:
            y = 1180 + int(lerp(16, 0, punch))
            rounded(d, (160, y, 920, y + 150), 26, mix(BG, (42, 28, 18), punch))
            d.text((W // 2, y + 75), "最后一个当回报", font=title_f, fill=mix(BG, YELLOW, punch), anchor="mm")
        out.append(img)
    return out
...
def render_b_cut(duration: float) -> list[Image.Image]:
    ...
    for i in range(n):
        ...
        tag(d, t, "推演 砍到三")
        a0 = appear(t, 0.02)
        d.text((W // 2, 230 + int(lerp(14, 0, a0))), "想到五个", font=title_f, fill=mix(BG, WHITE, a0), anchor="mm")

        for k, name in enumerate(cards):
            a = appear(t, 0.12 + k * 0.10, 0.18)
            if a < 0.04:
                continue
            y = 340 + k * 168 + int(lerp(16, 0, a))
            keep = k < 3
            cut = appear(t, 1.05, 0.24) if not keep else 0.0
            rounded(d, (130, y, 950, y + 146), 24, mix(BG, CARD, a))
            color = mix(CARD, MINT if keep else MUTED, a)
            d.text((200, y + 73), name, font=card_f, fill=color, anchor="lm")
            if keep:
                d.text((820, y + 73), "留", font=font(32), fill=mix(CARD, MINT, a), anchor="mm")
            elif cut > 0.04:
                d.text((820, y + 73), "砍", font=font(32), fill=mix(CARD, RED, cut), anchor="mm")
                strike_text(d, 200, y + 73, name, card_f, t, 1.12)

        punch = appear(t, 1.48, 0.22)
        if punch > 0.04:
            y = 1280 + int(lerp(16, 0, punch))
            rounded(d, (180, y, 900, y + 140), 26, mix(BG, (22, 40, 36), punch))
            d.text((W // 2, y + 70), "砍到三个", font=title_f, fill=mix(BG, MINT, punch), anchor="mm")
        out.append(img)
    return out


def render_b_unlink(duration: float) -> list[Image.Image]:
    n = max(1, round(duration * FPS))
    title_f = font(50)
    card_f = font(38)
    sub_f = font(28)
    ...
    for i in range(n):
        ...
        tag(d, t, "连不上 就假")
        a0 = appear(t, 0.02)
        d.text((W // 2, 220 + int(lerp(14, 0, a0))), "推得出来才留", font=title_f, fill=mix(BG, WHITE, a0), anchor="mm")

        law = appear(t, 0.10, 0.22)
        if law > 0.04:
            rounded(d, (200, 290, 880, 420), 26, mix(BG, CARD, law))
            d.text((W // 2, 355), "改的那条", font=card_f, fill=mix(CARD, YELLOW, law), anchor="mm")

        goods = [("后果一", 90), ("后果二", 390), ("后果三", 690)]
        for idx, (name, x) in enumerate(goods):
            a = appear(t, 0.28 + idx * 0.10, 0.20)
            if a < 0.04:
                continue
            y = 620
            d.line([(W // 2, 420), (x + 150, y)], fill=mix(BG, MINT, a), width=7)
            rounded(d, (x, y, x + 300, y + 200), 24, mix(BG, CARD, a))
            d.text((x + 150, y + 100), name, font=card_f, fill=mix(CARD, MINT, a), anchor="mm")

        fake = appear(t, 0.72, 0.22)
        if fake > 0.04:
            y = 920 + int(lerp(18, 0, fake))
            rounded(d, (160, y, 920, y + 200), 26, mix(BG, CARD, fake))
            d.text((W // 2, y + 70), "酷画面", font=card_f, fill=mix(CARD, WHITE, fake), anchor="mm")
            d.text((W // 2, y + 140), "推不出来", font=sub_f, fill=mix(CARD, RED, fake), anchor="mm")
            strike_text(d, 360, y + 70, "酷画面", card_f, t, 1.05)

        punch = appear(t, 1.38, 0.22)
        if punch > 0.04:
            y = 1220 + int(lerp(16, 0, punch))
            rounded(d, (150, y, 930, y + 150), 26, mix(BG, (42, 24, 22), punch))
            d.text((W // 2, y + 75), "观众判定假", font=title_f, fill=mix(BG, YELLOW, punch), anchor="mm")
        out.append(img)
    return out
...
def build_timeline(cues: list[tuple[float, float, str]], duration: float) -> dict:
    ...
    if len(cues) != 6:
        raise RuntimeError(f"need 6 phrases, got {len(cues)}")
    p0, p1, p2, p3, p4, p5 = cues
    b1 = max(p0[1] + 1.05, p2[0] - LEAD)
    b2 = max(b1 + 1.15, p3[0] - LEAD)
    b3 = max(b2 + 1.15, p4[0] - LEAD)
    if b1 <= p0[1] + 0.4:
        b1 = p2[0]
    if b2 <= b1 + 0.8:
        b2 = p3[0]
    ...
    shots = [
        ...
        {"id": "S02", "kind": "B", "start": round(b1, 3), "end": round(b2, 3), "src": "broll/B-定律三后果.mp4", "line": p2[2], "broll": "three_from_law"},
        {"id": "S03", "kind": "B", "start": round(b2, 3), "end": round(b3, 3), "src": "broll/B-五个砍三个.mp4", "line": p3[2], "broll": "cut_to_three"},
        {"id": "S04", "kind": "B", "start": round(b3, 3), "end": round(p4[1], 3), "src": "broll/B-连不上就删.mp4", "line": p4[2], "broll": "unlink_fake"},
    ...
    eyebrows = [
        ...
        {"start": round(p5[0], 3), "end": round(duration, 3), "text": "A-ROLL / 05"},
    ]
    data = {
...
def make_cover() -> Path:
    ...
    d.text((W // 2, 160), "推不出来", font=font(58), fill=WHITE, anchor="mm")
    d.text((W // 2, 250), "一律删掉", font=font(64), fill=YELLOW, anchor="mm")
...
def write_docs(duration: float, staged: Path, data: dict) -> None:
    ...
    status = {
        "schema_version": 1,
        "project_name": "39_推不出来的酷画面删掉",
        "video_type": "普通短视频",
        "episode": 39,
        "title": NAME,
        "source_note": "topics-batch3.md #39 / 选题库.md 开篇推演纪律",
        "unused_of": "成片00-37 / 成片38对照卡 / batch3#38分不清改物理",
        "voice": VOICE,
        "duration": round(duration, 3),
        "size": [W, H],
        "fps": FPS,
...
def main() -> None:
    ...
    b3 = next(s for s in data["shots"] if s["id"] == "S04")
    d1 = max(2.2, float(b1["end"]) - float(b1["start"]))
    d2 = max(2.2, float(b2["end"]) - float(b2["start"]))
    d3 = max(2.2, float(b3["end"]) - float(b3["start"]))
    print("render B-定律三后果", d1)
    frames_to_mp4(render_b_law(d1 + 0.12), ROOT / "broll" / "B-定律三后果.mp4")
    print("render B-五个砍三个", d2)
    frames_to_mp4(render_b_cut(d2 + 0.12), ROOT / "broll" / "B-五个砍三个.mp4")
    print("render B-连不上就删", d3)
    frames_to_mp4(render_b_unlink(d3 + 0.12), ROOT / "broll" / "B-连不上就删.mp4")
