#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Topic 45 / topics-batch3 #45：会议没有结束时间。云端 A-roll + B-roll。

普通短视频 / 知识口播。草稿只写 .abroll-cloud/45/，成品中转 成片/45-会议没有结束时间.mp4。
...
def render_b_talk(duration: float) -> list[Image.Image]:
    ...
    for i in range(n):
        ...
        tag(d, t, "错 · 没写结束")
        a0 = appear(t, 0.02)
        d.text((W // 2, 236 + int(lerp(18, 0, a0))), "没写结束", font=title_f, fill=mix(BG, WHITE, a0), anchor="mm")

        a1 = appear(t, 0.16)
        y = 340 + int(lerp(24, 0, a1))
        rounded(d, (72, y, 508, y + 420), 28, mix(BG, CARD, a1))
        d.text((290, y + 78), "日历 · 开始", font=sub_f, fill=mix(CARD, MUTED, a1), anchor="mm")
        d.text((290, y + 190), "9:00", font=card_f, fill=mix(CARD, MINT, a1), anchor="mm")
        d.text((290, y + 300), "写了就开始", font=sub_f, fill=mix(CARD, MUTED, a1), anchor="mm")
        check_badge(d, 430, y + 70, appear(t, 0.42, 0.22))

        a2 = appear(t, 0.28)
        rounded(d, (572, y, 1008, y + 420), 28, mix(BG, CARD, a2))
        d.text((790, y + 78), "日历 · 结束", font=sub_f, fill=mix(CARD, MUTED, a2), anchor="mm")
        d.text((790, y + 190), "空", font=card_f, fill=mix(CARD, RED, a2), anchor="mm")
        d.text((790, y + 300), "没人敢先走", font=sub_f, fill=mix(CARD, MUTED, a2), anchor="mm")
        strike_box(d, (620, y + 150, 960, y + 230), appear(t, 0.85, 0.28), mix(CARD, RED, appear(t, 0.85, 0.28)))

        punch = appear(t, 1.28, 0.26)
        if punch > 0.04:
...
def render_b_end(duration: float) -> list[Image.Image]:
    ...
    for i in range(n):
        ...
        tag(d, t, "对 · 散会点")
        a0 = appear(t, 0.02)
        d.text((W // 2, 236 + int(lerp(18, 0, a0))), "预约先写下散会点", font=title_f, fill=mix(BG, WHITE, a0), anchor="mm")

        a1 = appear(t, 0.16)
        y = 340 + int(lerp(24, 0, a1))
        rounded(d, (90, y, 990, y + 280), 32, mix(BG, CARD, a1))
        d.text((W // 2, y + 90), "散会 9:40", font=card_f, fill=mix(CARD, YELLOW, a1), anchor="mm")
        d.text((W // 2, y + 190), "预约时就钉死", font=sub_f, fill=mix(CARD, MUTED, a1), anchor="mm")
        check_badge(d, 900, y + 70, appear(t, 0.48, 0.22))

        a2 = appear(t, 0.72)
        y2 = 680 + int(lerp(24, 0, a2))
        rounded(d, (90, y2, 990, y2 + 260), 32, mix(BG, (22, 40, 36), a2))
        d.text((W // 2, y2 + 90), "到点没有结论", font=card_f, fill=mix(CARD, WHITE, a2), anchor="mm")
        d.text((W // 2, y2 + 180), "也先散，结论另约", font=sub_f, fill=mix(CARD, MUTED, a2), anchor="mm")
...
def build_timeline(cues: list[tuple[float, float, str]], duration: float) -> dict:
    ...
    shots = [
        ...
        {"id": "S02", "kind": "B", "start": round(b1, 3), "end": round(p2[1], 3), "src": "broll/B-一直聊.mp4", "line": p2[2], "broll": "keep_talking"},
        {"id": "S03", "kind": "B", "start": round(p2[1], 3), "end": round(b2, 3), "src": "broll/B-散会点.mp4", "line": p3[2], "broll": "end_slot"},
...
def write_docs(duration: float, staged: Path, data: dict) -> None:
    ...
    status = {
        "schema_version": 1,
        "project_name": "45_会议没有结束时间",
        "video_type": "普通短视频",
        "episode": 45,
        "title": NAME,
        "source_note": "topics-batch3.md #45 / 工厂 会议没有结束时间 voiceover.txt",
        "slug": "会议没有结束时间",
        "voice": VOICE,
        "duration": round(duration, 3),
        "size": [W, H],
...
def main() -> None:
    ...
    print("render B-一直聊", d1)
    frames_to_mp4(render_b_talk(d1 + 0.12), ROOT / "broll" / "B-一直聊.mp4")
    print("render B-散会点", d2)
    frames_to_mp4(render_b_end(d2 + 0.12), ROOT / "broll" / "B-散会点.mp4")
