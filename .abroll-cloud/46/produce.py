#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""成片 46：杂务按六十分及格。备忘录第二节钝感。云端 A-roll + B-roll，不是短剧。"""
...
def render_b_rent(duration: float) -> list[Image.Image]:
    """考卷六十分盖章「过」；杂务是换工位的租金。"""
    ...
    for i in range(n):
        ...
        tag(d, t, "对 · 租金")
        a0 = appear(t, 0.02)
        d.text((W // 2, 236 + int(lerp(16, 0, a0))), "换工位的租金", font=font(58), fill=mix(BG, WHITE, a0), anchor="mm")

        a1 = appear(t, 0.16)
        if a1 > 0.04:
            y = 330 + int(lerp(22, 0, a1))
            rounded(d, (80, y, 1000, y + 520), 36, mix(BG, CARD, a1))
            d.text((W // 2, y + 70), "行政杂务", font=font(36), fill=mix(CARD, MUTED, a1), anchor="mm")
            score_a = appear(t, 0.42, 0.35)
            score = int(lerp(12, 60, score_a))
            d.text((W // 2, y + 220), f"{score}", font=font(160), fill=mix(CARD, MINT if score >= 60 else YELLOW, a1), anchor="mm")
            d.text((W // 2, y + 360), "分", font=font(40), fill=mix(CARD, MUTED, a1), anchor="mm")
            stamp = appear(t, 1.05, 0.28)
            if stamp > 0.04:
                cx, cy = 820, y + 150
                col = mix(CARD, MINT, stamp)
                d.ellipse((cx - 88, cy - 88, cx + 88, cy + 88), outline=col, width=10)
                d.text((cx, cy), "过", font=font(72), fill=col, anchor="mm")
            d.text((W // 2, y + 450), "不出差错即可", font=font(36), fill=mix(CARD, YELLOW, a1), anchor="mm")

        punch = appear(t, 1.42, 0.24)
        if punch > 0.04:
            y = 920 + int(lerp(22, 0, punch))
            rounded(d, (120, y, 960, y + 200), 28, mix(BG, (18, 42, 36), punch))
            d.text((W // 2, y + 100), "六十分就过", font=font(52), fill=mix(BG, MINT, punch), anchor="mm")
        out.append(img)
    return out


def render_b_honor(duration: float) -> list[Image.Image]:
    """评优争先打叉；卫生清单及格勾；做完即止。"""
    n = max(1, round(duration * FPS))
    out = []
    for i in range(n):
        ...
        if a1 > 0.04:
            ...
            d.text((300, y + 170), "评优争先", font=font(44), fill=mix(CARD, YELLOW, a1), anchor="mm")
            d.text((300, y + 270), "情绪成本白投", font=font(32), fill=mix(CARD, MUTED, a1), anchor="mm")
            strike_line(d, (140, y + 130, 460, y + 210), appear(t, 0.88, 0.28), mix(CARD, RED, a1))
            xmark = appear(t, 1.05, 0.22)
            if xmark > 0.04:
                col = mix(CARD, RED, xmark)
                d.line([(240, y + 320), (360, y + 400)], fill=col, width=10)
                d.line([(360, y + 320), (240, y + 400)], fill=col, width=10)

        a2 = appear(t, 0.28)
        if a2 > 0.04:
            y = 330 + int(lerp(20, 0, a2))
            rounded(d, (560, y, 1000, y + 420), 32, mix(BG, (22, 40, 36), a2))
            d.text((780, y + 80), "勾", font=font(30), fill=mix(CARD, MUTED, a2), anchor="mm")
            d.text((780, y + 170), "做完即止", font=font(44), fill=mix(CARD, MINT, a2), anchor="mm")
            items = ["工位卫生", "收发快递", "盖章跑腿"]
            for j, item in enumerate(items):
                iy = y + 240 + j * 52
                box = (620, iy, 940, iy + 42)
                rounded(d, box, 10, mix(CARD, (18, 28, 26), a2))
                d.text((670, iy + 21), item, font=font(28), fill=mix(CARD, WHITE, a2), anchor="lm")
                chk = appear(t, 0.95 + j * 0.12, 0.18)
                if chk > 0.04:
                    col = mix(CARD, MINT, chk)
                    d.line([(622, iy + 22), (634, iy + 32), (652, iy + 10)], fill=col, width=5)

        punch = appear(t, 1.40, 0.24)
        if punch > 0.04:
            y = 820 + int(lerp(20, 0, punch))
...
def build_timeline(cues: list[tuple[float, float, str]], duration: float) -> dict:
    ...
    shots = [
        ...
        {"id": "S02", "kind": "B", "start": b1, "end": p2[1], "src": "broll/B-租金六十分.mp4", "line": p2[2], "broll": "rent_sixty"},
        {"id": "S03", "kind": "A", "start": p2[1], "end": b2, "src": "assets/V-指向.mp4", "line": p3[2]},
        {"id": "S04", "kind": "B", "start": b2, "end": duration, "src": "broll/B-别评优.mp4", "line": p4[2], "broll": "no_honor"},
    ]
    ...
    data = {
        ...
        "a_caps": [
            ...
            {"start": round(p1[0], 3), "end": round(b1, 3), "lines": ["杂务按六十分", "及格交付"]},
            {"start": round(p3[0], 3), "end": round(b2, 3), "lines": ["做完即止，不出差错"]},
        ],
        "shutters": [
...
def write_docs(duration: float, staged: Path, data: dict) -> None:
    ...
    status = {
        "schema_version": 1,
        "project_name": "46_杂务按六十分及格",
        "video_type": "普通短视频",
        "episode": 46,
        "title": NAME,
        "full_title": FULL_TITLE,
        "source_note": "topics-batch3.md #46 · 备忘录第二节 2 钝感策略",
        "slug": "杂务按六十分及格交付",
        "voice": VOICE,
        "duration": round(duration, 3),
        "size": [W, H],
...
def main() -> None:
    ...
    print("render B-租金六十分", d1)
    frames_to_mp4(render_b_rent(d1 + 0.12), ROOT / "broll" / "B-租金六十分.mp4")
    print("render B-别评优", d2)
    frames_to_mp4(render_b_honor(d2 + 0.12), ROOT / "broll" / "B-别评优.mp4")
