#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""成片 47：研一别脱产离校实习。云端 A-roll + B-roll，不是短剧。

topics-batch3.md #47。备忘录第二节 3。禁止 C:\\ D:\\ G:\\，禁止 Drive 上传。
"""
...
def draw_empty_desk(draw, cx: int, cy: int, scale: float, a: float) -> None:
    ...
    draw.rectangle((cx0 + 10, cy0 + 12, cx0 + cw - 10, cy0 + 22), fill=mix(CARD, MUTED, a))
    fnt = font(max(18, int(22 * scale)))
    draw.text((cx, y0 + int(18 * scale)), "工位", font=fnt, fill=mix(CARD, MUTED, a), anchor="mm")
    draw.text((cx, cy0 + ch + int(22 * scale)), "空着", font=fnt, fill=mix(BG, RED, a), anchor="mm")
...
def draw_roster(draw, cx: int, cy: int, scale: float, a: float, marked: bool) -> None:
    ...
    fnt = font(max(18, int(22 * scale)))
    draw.text((cx, y0 + int(24 * scale)), "组会名单", font=fnt, fill=mix(YELLOW, INK, a), anchor="mm")
    names = ["甲", "乙", "丙", "丁"]
    row_f = font(max(20, int(26 * scale)))
    for i, name in enumerate(names):
        yy = y0 + int(72 * scale) + i * int(42 * scale)
        col = mix(CARD, WHITE, a)
        draw.text((x0 + int(36 * scale), yy), name, font=row_f, fill=col, anchor="lm")
        if marked and i == 2:
...
def draw_ticket(draw, cx: int, cy: int, scale: float, a: float, stamped: bool) -> None:
    ...
    title_f = font(max(22, int(28 * scale)))
    sub_f = font(max(20, int(26 * scale)))
    draw.text((cx, y0 + int(48 * scale)), "合肥  →  外地", font=title_f, fill=mix(CARD, WHITE, a), anchor="mm")
    draw.text((cx, y0 + int(110 * scale)), "全职离校", font=sub_f, fill=mix(CARD, MUTED, a), anchor="mm")
    if stamped:
        ...
        draw.text((rx0 + int(60 * scale), ry0 + int(60 * scale)), "先别", font=title_f, fill=mix(CARD, RED, a), anchor="mm")


def render_b_checkpoint(duration: float) -> list[Image.Image]:
    n = max(1, round(duration * FPS))
    title_f = font(56)
    card_f = font(40)
    sub_f = font(28)
...
def write_docs(duration: float, staged: Path, data: dict) -> None:
    ...
    status = {
        "schema_version": 1,
        "project_name": "47_研一别脱产离校实习",
        "video_type": "普通短视频",
        "episode": 47,
        "title": NAME,
        "source_note": "topics-batch3.md #47 / 备忘录第二节 3 脱产全职离校实习",
        "unused_of": "工位切片（成片16）/ 报销红线（成片28）/ 组会撞课（成片37）/ 杂务六十分（#46）/ 远程实习（#48）",
        "voice": VOICE,
        "duration": round(duration, 3),
        "size": [W, H],
        "fps": FPS,
        ...
        "windows_paths": False,
        "drive_upload": False,
        "not": "drama-pipeline / 仙侠连载 / 远程实习讲法 / 下一期预告 / C: D: G: / Drive 上传",
        "updated_at": now,
    }
...
def patch_delivery(duration: float) -> None:
    ...
    marker = "| 47 | — | 待收 |"
    row = f"| 47 | `{STAGED_NAME}` | 已核验 {duration:.1f}s |"
    if marker in text:
...
def patch_topics(duration: float) -> None:
    ...
    if not path.exists():
        return
    text = path.read_text(encoding="utf-8")
    old = "| 47 | 专硕实习红线 | `47-研一别脱产离校实习.mp4` | 未拍 |"
    new = f"| 47 | 专硕实习红线 | `{STAGED_NAME}` | 已核验 {duration:.1f}s |"
    if old in text:
