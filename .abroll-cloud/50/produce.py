#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""成片 50：绑定一个主力博士。topics-batch3 #50。云端 A-roll + B-roll，不是短剧。"""
...
def render_b_one(duration: float) -> list[Image.Image]:
    ...
    for i in range(n):
        ...
        tag(d, t, "对 · 一个")
        a0 = appear(t, 0.02)
        d.text((W // 2, 236 + int(lerp(16, 0, a0))), "认准一位主力", font=font(56), fill=mix(BG, WHITE, a0), anchor="mm")

        a1 = appear(t, 0.16)
        if a1 > 0.04:
            y = 320 + int(lerp(22, 0, a1))
            rounded(d, (70, y, 500, y + 520), 32, mix(BG, CARD, a1))
            d.text((285, y + 56), "广撒网", font=font(36), fill=mix(CARD, MUTED, a1), anchor="mm")
            names = ["甲", "乙", "丙", "丁"]
            for j, name in enumerate(names):
                ...
                if xmark > 0.04:
                    ...
                    d.line([(cx - 28, cy - 40), (cx + 28, cy + 20)], fill=col, width=8)
                    d.line([(cx + 28, cy - 40), (cx - 28, cy + 20)], fill=col, width=8)
            strike_line(d, (110, y + 30, 460, y + 86), appear(t, 0.88, 0.28), mix(CARD, RED, a1))

        a2 = appear(t, 0.28)
        if a2 > 0.04:
            y = 320 + int(lerp(22, 0, a2))
            rounded(d, (540, y, 1010, y + 520), 32, mix(BG, (22, 40, 36), a2))
            d.text((775, y + 56), "主力博士", font=font(36), fill=mix(CARD, MINT, a2), anchor="mm")
            draw_person(d, 775, y + 210, a2, MINT, "稳")
            ring = appear(t, 0.70, 0.28)
            if ring > 0.04:
                col = mix(CARD, YELLOW, ring)
                d.ellipse((775 - 78, y + 116, 775 + 78, y + 272), outline=col, width=8)
            d.text((775, y + 360), "有论文任务", font=font(34), fill=mix(CARD, YELLOW, a2), anchor="mm")
            d.text((775, y + 430), "高年级 · 一条线", font=font(28), fill=mix(CARD, MUTED, a2), anchor="mm")

        punch = appear(t, 1.42, 0.24)
        if punch > 0.04:
            y = 900 + int(lerp(22, 0, punch))
            rounded(d, (120, y, 960, y + 200), 28, mix(BG, (18, 42, 36), punch))
            d.text((W // 2, y + 100), "只绑一个", font=font(56), fill=mix(BG, MINT, punch), anchor="mm")
        out.append(img)
    return out
...
def render_b_swap(duration: float) -> list[Image.Image]:
    ...
    for i in range(n):
        ...
        if punch > 0.04:
            ...
            rounded(d, (120, y, 960, y + 200), 28, mix(BG, (18, 42, 36), punch))
            d.text((W // 2, y + 100), "结成利益同盟", font=font(52), fill=mix(BG, MINT, punch), anchor="mm")
        out.append(img)
    return out
...
def build_timeline(cues: list[tuple[float, float, str]], duration: float) -> dict:
    ...
    shots = [
        ...
        {"id": "S02", "kind": "B", "start": b1, "end": p2[1], "src": "broll/B-只绑一个.mp4", "line": p2[2], "broll": "one_phd"},
        {"id": "S03", "kind": "A", "start": p2[1], "end": b2, "src": "assets/V-指向.mp4", "line": p3[2]},
        {"id": "S04", "kind": "B", "start": b2, "end": duration, "src": "broll/B-换署名数据.mp4", "line": p4[2], "broll": "authorship"},
    ]
    ...
    data = {
        ...
        "a_caps": [
            ...
            {"start": round(p1[0], 3), "end": round(b1, 3), "lines": ["绑定一个主力博士"]},
            {"start": round(p3[0], 3), "end": round(b2, 3), "lines": ["搭自动化、处理数据"]},
        ],
        "shutters": [
...
def main() -> None:
    ...
    print("render B-只绑一个", d1)
    frames_to_mp4(render_b_one(d1 + 0.12), ROOT / "broll" / "B-只绑一个.mp4")
    print("render B-换署名数据", d2)
    frames_to_mp4(render_b_swap(d2 + 0.12), ROOT / "broll" / "B-换署名数据.mp4")
