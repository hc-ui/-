#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""成片 51：Agent是第二技能树不是主线。topics-batch3 #51。云端 A-roll + B-roll，不是短剧。"""
...
def make_cover() -> Path:
    ...
    d.text((W // 2, 160), "Agent是", font=font(48), fill=WHITE, anchor="mm")
    d.text((W // 2, 250), "第二技能树", font=font(64), fill=YELLOW, anchor="mm")
...
def qa(staged: Path, data: dict) -> dict:
    ...
    report = {
        "project": "51_Agent是第二技能树不是主线",
        "strict": True,
        "cloud_only": True,
        "windows_paths": False,
        "drive_upload": False,
        "video_type": "普通短视频",
        "topic": "topics-batch3.md #51",
        "final": str(staged),
