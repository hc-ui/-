...
def build_timeline(cues: list[tuple[float, float, str]], duration: float) -> dict:
    ...
    shots = [
        ...
        {"id": "S01b", "kind": "A", "start": p0[1], "end": b1, "src": "assets/V-正对讲.mp4", "line": p1[2]},
        {"id": "S02", "kind": "B", "start": b1, "end": p2[1], "src": "broll/B-钩子没咬住.mp4", "line": p2[2], "broll": "hook_miss"},
        {"id": "S03", "kind": "A", "start": p2[1], "end": b2, "src": "assets/V-指向.mp4", "line": p3[2]},
        {"id": "S04", "kind": "B", "start": b2, "end": duration, "src": "broll/B-问号留空.mp4", "line": p4[2], "broll": "leave_beat"},
    ]
    ...
    data = {
        ...
        "a_caps": [
            ...
            {"start": round(p1[0], 3), "end": round(b1, 3), "lines": ["提问后面留半拍"]},
            {"start": round(p3[0], 3), "end": round(b2, 3), "lines": ["问句落地", "停半拍再给答案"]},
        ],
        "shutters": [
...
def patch_index(duration: float) -> None:
    ...
    for idx in (Path("/workspace/成片/INDEX.md"), Path("/workspace/.abroll-cloud/INDEX.chengpian.md")):
        ...
        if STAGED_NAME in text:
            text = re.sub(rf"\| `{re.escape(STAGED_NAME)}` \| [0-9.]+s \|", line, text)
...
def main() -> None:
    ...
    print("render B-钩子没咬住", d1)
    frames_to_mp4(render_b_hook(d1 + 0.12), ROOT / "broll" / "B-钩子没咬住.mp4")
    print("render B-问号留空", d2)
    frames_to_mp4(render_b_wait(d2 + 0.12), ROOT / "broll" / "B-问号留空.mp4")
