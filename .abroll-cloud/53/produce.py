#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""成片 53：提问后面留半拍。工厂 340 口播工艺。云端 A-roll + B-roll，不是短剧。"""
...
NAME = "提问后面留半拍"
STAGED_NAME = "53-提问后面留半拍.mp4"
VOICE = "zh-CN-YunyangNeural"
ASSET_SRC = Path("/workspace/.abroll-cloud/06/assets")
TALK_SRC = Path("/workspace/.abroll-cloud/25/assets")
...
def frames_to_mp4(frames, dest: Path) -> None:
    ...
    if kept is not None:
        kept.save(dest.with_suffix(".jpg"), quality=92)


def dur_zh(seconds: float) -> str:
...
def cross_badge(draw: ImageDraw.ImageDraw, cx: int, cy: int, a: float) -> None:
    if a <= 0.04:
        return
    r = 36
...
def wave_bar(x: float, t: float, gap_at: float | None, gap_w: float) -> float:
    """Return 0–1 bar height. gap_at is the ? marker in 0–1 along the strip."""
    if gap_at is not None and abs(x - gap_at) < gap_w:
        return 0.06
    env = 0.35 + 0.65 * abs(math.sin(x * 18.0 + t * 7.4))
    if x < 0.18:
        env *= x / 0.18
    return env


def draw_waveform(draw: ImageDraw.ImageDraw, box, t: float, a: float, gap_at: float | None) -> None:
    x0, y0, x1, y1 = box
    mid = (y0 + y1) / 2
    n = 36
    gap_w = 0.055
    for i in range(n):
        x = (i + 0.5) / n
        h = wave_bar(x, t, gap_at, gap_w)
        bh = int((y1 - y0) * 0.42 * h)
        cx = int(lerp(x0 + 16, x1 - 16, x))
        col = mix(CARD, MINT if gap_at is None or abs(x - gap_at) > gap_w else YELLOW, a)
        draw.rectangle((cx - 6, mid - bh, cx + 6, mid + bh), fill=col)
    if gap_at is not None:
        qx = int(lerp(x0 + 16, x1 - 16, gap_at))
        draw.text((qx, y0 + 8), "?", font=font(28), fill=mix(CARD, YELLOW, a), anchor="mt")


def render_b_hook(duration: float) -> list[Image.Image]:
    """错：问完立刻自答，波形不断，钩子滑走。"""
    ...
    for i in range(n):
        ...
        tag(d, t, "错 · 立刻答")
        a0 = appear(t, 0.02)
        d.text((W // 2, 236 + int(lerp(16, 0, a0))), "问完立刻自答", font=font(56), fill=mix(BG, WHITE, a0), anchor="mm")

        a1 = appear(t, 0.14)
        if a1 > 0.04:
            y = 320 + int(lerp(22, 0, a1))
            rounded(d, (80, y, 1000, y + 300), 32, mix(BG, CARD, a1))
            draw_waveform(d, (120, y + 36, 960, y + 168), t, a1, gap_at=None)
            d.text((W // 2, y + 230), "问号后没空格", font=font(36), fill=mix(CARD, MUTED, a1), anchor="mm")
            cross_badge(d, 930, y + 54, appear(t, 0.42, 0.22))

        a2 = appear(t, 0.36)
        if a2 > 0.04:
            y = 670 + int(lerp(20, 0, a2))
            slide = int(lerp(0, 48, appear(t, 1.05, 0.40)))
            rounded(d, (80 + slide, y, 1000 + slide, y + 280), 32, mix(BG, CARD, a2))
            d.text((W // 2 + slide, y + 80), "钩子没咬住", font=font(48), fill=mix(CARD, YELLOW, a2), anchor="mm")
            d.text((W // 2 + slide, y + 180), "人已经滑走", font=font(40), fill=mix(CARD, RED, a2), anchor="mm")
            strike_line(d, (180 + slide, y + 40, 900 + slide, y + 120), appear(t, 1.10, 0.28), mix(CARD, RED, a2))

        punch = appear(t, 1.42, 0.24)
        if punch > 0.04:
            y = 1010 + int(lerp(22, 0, punch))
            rounded(d, (120, y, 960, y + 200), 28, mix(BG, (42, 24, 22), punch))
            d.text((W // 2, y + 100), "钩子没咬住", font=font(52), fill=mix(BG, YELLOW, punch), anchor="mm")
        out.append(img)
    return out


def render_b_wait(duration: float) -> list[Image.Image]:
    """对：问号后空半格，停半拍再给答案。"""
    n = max(1, round(duration * FPS))
    out = []
    for i in range(n):
        ...
        tag(d, t, "对 · 留半拍")
        a0 = appear(t, 0.02)
        d.text((W // 2, 236 + int(lerp(16, 0, a0))), "问句落地再给答案", font=font(50), fill=mix(BG, WHITE, a0), anchor="mm")

        a1 = appear(t, 0.14)
        if a1 > 0.04:
            y = 320 + int(lerp(20, 0, a1))
            rounded(d, (80, y, 1000, y + 300), 32, mix(BG, CARD, a1))
            draw_waveform(d, (120, y + 36, 960, y + 168), t, a1, gap_at=0.46)
            d.text((W // 2, y + 230), "波形停半拍", font=font(36), fill=mix(CARD, YELLOW, a1), anchor="mm")

        a2 = appear(t, 0.32)
        if a2 > 0.04:
            y = 670 + int(lerp(20, 0, a2))
            rounded(d, (80, y, 500, y + 300), 32, mix(BG, (22, 40, 36), a2))
            d.text((290, y + 70), "字幕", font=font(28), fill=mix(CARD, MUTED, a2), anchor="mm")
            d.text((290, y + 150), "为什么？", font=font(44), fill=mix(CARD, WHITE, a2), anchor="mm")
            ans = appear(t, 1.05, 0.28)
            if ans > 0.08:
                d.text((290, y + 230), "再给答案", font=font(36), fill=mix(CARD, MINT, ans), anchor="mm")
            else:
                d.text((290, y + 230), "· · ·", font=font(36), fill=mix(CARD, MUTED, a2), anchor="mm")

            rounded(d, (580, y, 1000, y + 300), 32, mix(BG, CARD, a2))
            d.text((790, y + 70), "口播", font=font(28), fill=mix(CARD, MUTED, a2), anchor="mm")
            d.text((790, y + 150), "停半拍", font=font(44), fill=mix(CARD, MINT, a2), anchor="mm")
            d.text((790, y + 230), "再答", font=font(36), fill=mix(CARD, WHITE, a2), anchor="mm")
            check_badge(d, 930, y + 64, appear(t, 0.88, 0.22))

        punch = appear(t, 1.42, 0.24)
        if punch > 0.04:
            y = 1030 + int(lerp(20, 0, punch))
            rounded(d, (120, y, 960, y + 200), 28, mix(BG, (18, 42, 36), punch))
            d.text((W // 2, y + 100), "问号后面留空", font=font(52), fill=mix(BG, MINT, punch), anchor="mm")
        out.append(img)
    return out


def render_b_slide(duration: float) -> list[Image.Image]:
    """例一：为什么因为粘一行，答案砸下来，手指滑走。"""
    n = max(1, round(duration * FPS))
    out = []
    for i in range(n):
        t = i / FPS
        ...
        tag(d, t, "例 · 连着答")
        a0 = appear(t, 0.02)
        d.text((W // 2, 236 + int(lerp(16, 0, a0))), "问号后面连着自己答", font=font(48), fill=mix(BG, WHITE, a0), anchor="mm")

        a1 = appear(t, 0.14)
        if a1 > 0.04:
            y = 320 + int(lerp(20, 0, a1))
            rounded(d, (80, y, 1000, y + 280), 32, mix(BG, CARD, a1))
            d.text((W // 2, y + 80), "为什么因为", font=font(56), fill=mix(CARD, YELLOW, a1), anchor="mm")
            d.text((W // 2, y + 180), "问号和答案粘在一行", font=font(36), fill=mix(CARD, MUTED, a1), anchor="mm")
            strike_line(d, (180, y + 40, 900, y + 120), appear(t, 0.95, 0.28), mix(CARD, RED, a1))

        a2 = appear(t, 0.40)
        if a2 > 0.04:
            y = 640 + int(lerp(20, 0, a2))
            slide = int(lerp(0, 120, appear(t, 1.10, 0.45)))
            rounded(d, (80 + slide, y, 1000 + slide, y + 260), 32, mix(BG, CARD, a2))
            d.text((W // 2 + slide, y + 80), "答案已经砸下来", font=font(46), fill=mix(CARD, RED, a2), anchor="mm")
            d.text((W // 2 + slide, y + 170), "人还没点头", font=font(40), fill=mix(CARD, YELLOW, a2), anchor="mm")

        punch = appear(t, 1.55, 0.24)
        if punch > 0.04:
            y = 960 + int(lerp(20, 0, punch))
            rounded(d, (120, y, 960, y + 200), 28, mix(BG, (42, 24, 22), punch))
            d.text((W // 2, y + 100), "钩子等于没抛", font=font(52), fill=mix(BG, YELLOW, punch), anchor="mm")
        out.append(img)
    return out
...
def build_timeline(cues: list[tuple[float, float, str]], duration: float) -> dict:
    ...
    if len(cues) != 12:
        raise SystemExit(f"need 12 cues, got {len(cues)}: {cues}")
    p = cues
    b_hook = max(p[1][1], p[2][0] - LEAD)
    b_slide = max(p[3][1], p[4][0] - LEAD)
    b_wait = max(p[6][1], p[7][0] - LEAD)
    a_close = p[9][1]
    shots = [
        {"id": "S01a", "kind": "A", "start": 0.0, "end": p[0][1], "src": "assets/V-挥手.mp4", "line": p[0][2], "close": True},
        {"id": "S01b", "kind": "A", "start": p[0][1], "end": b_hook, "src": "assets/V-正对讲.mp4", "line": p[1][2]},
        {"id": "S02", "kind": "B", "start": b_hook, "end": p[2][1], "src": "broll/B-钩子没咬住.mp4", "line": p[2][2], "broll": "hook_miss"},
        {"id": "S03", "kind": "A", "start": p[2][1], "end": b_slide, "src": "assets/V-指向.mp4", "line": p[3][2]},
        {"id": "S04", "kind": "B", "start": b_slide, "end": p[5][1], "src": "broll/B-人滑走.mp4", "line": p[4][2] + p[5][2], "broll": "slide_away"},
        {"id": "S05", "kind": "A", "start": p[5][1], "end": b_wait, "src": "assets/V-指向.mp4", "line": p[6][2]},
        {"id": "S06", "kind": "B", "start": b_wait, "end": p[8][1], "src": "broll/B-问号留空.mp4", "line": p[7][2] + p[8][2], "broll": "leave_beat"},
        {"id": "S07", "kind": "A", "start": p[8][1], "end": a_close, "src": "assets/V-摊手.mp4", "line": p[9][2]},
        {"id": "S08", "kind": "A", "start": a_close, "end": duration, "src": "assets/V-点赞.mp4", "line": p[10][2] + p[11][2]},
    ]
    ...
    data = {
        ...
        "a_caps": [
            ...
            {"start": round(p[1][0], 3), "end": round(b_hook, 3), "lines": ["提问后面留半拍"]},
            {"start": round(p[3][0], 3), "end": round(b_slide, 3), "lines": split_caption(p[3][2])},
            {"start": round(p[6][0], 3), "end": round(b_wait, 3), "lines": ["问句落地", "停半拍再给答案"]},
            {"start": round(p[9][0], 3), "end": round(a_close, 3), "lines": ["写完口播", "在问号后面留空"]},
            {"start": round(p[10][0], 3), "end": round(duration, 3), "lines": ["半拍不是冷场", "问完先停再答"]},
        ],
        "shutters": [
            {"start": round(p[0][1], 3), "color": list(CREAM)},
            {"start": round(b_hook, 3), "color": list(MINT)},
            {"start": round(p[2][1], 3), "color": list(CREAM)},
            {"start": round(b_slide, 3), "color": list(YELLOW)},
            {"start": round(p[5][1], 3), "color": list(CREAM)},
            {"start": round(b_wait, 3), "color": list(MINT)},
            {"start": round(p[8][1], 3), "color": list(CREAM)},
            {"start": round(a_close, 3), "color": list(YELLOW)},
        ],
        "eyebrows": [
            {"start": 0.0, "end": round(p[0][1], 3), "text": "A-ROLL / 1a"},
            {"start": round(p[0][1], 3), "end": round(b_hook, 3), "text": "A-ROLL / 1b"},
            {"start": round(p[2][1], 3), "end": round(b_slide, 3), "text": "A-ROLL / 03"},
            {"start": round(p[5][1], 3), "end": round(b_wait, 3), "text": "A-ROLL / 05"},
            {"start": round(p[8][1], 3), "end": round(a_close, 3), "text": "A-ROLL / 07"},
            {"start": round(a_close, 3), "end": round(duration, 3), "text": "A-ROLL / 08"},
        ],
        "cover": {
        ...
        "b_lead_s": LEAD,
        "video_type": "普通短视频",
        "source_note": "topics-batch3.md #53 / 工厂 340_提问后面留半拍",
        "length_rule": "30-60秒，目标约40秒",
    }
...
def make_cover() -> Path:
    ...
    d.text((W // 2, 160), "提问后面", font=font(48), fill=WHITE, anchor="mm")
    d.text((W // 2, 250), "留半拍", font=font(64), fill=YELLOW, anchor="mm")
...
def copy_assets() -> None:
    ...
    mapping = {
        "V-挥手.mp4": ASSET_SRC / "V-挥手.mp4",
        "V-指向.mp4": ASSET_SRC / "V-指向.mp4",
        "V-摊手.mp4": ASSET_SRC / "V-摊手.mp4",
        "V-点赞.mp4": ASSET_SRC / "V-点赞.mp4",
        "V-正对讲.mp4": TALK_SRC / "V-正对讲.mp4",
        "A-角色-小灯-摊手.jpg": ASSET_SRC / "A-角色-小灯-摊手.jpg",
    }
...
def qa(staged: Path, data: dict) -> dict:
    ...
    report = {
        ...
        "duration_zh": dur_zh(float(info["format"]["duration"])),
        "length_ok": 30.0 <= float(info["format"]["duration"]) <= 60.0,
        "ok": True,
    }
    report["ok"] = (
...
def main() -> None:
    copy_assets()
    audio_dir = ROOT / "audio"
    for name in ("vo-full.wav", "vo-full.mp3", "vo-align.txt"):
        p = audio_dir / name
        if p.exists():
            p.unlink()
    duration, cues = make_voiceover()
    print("VO", duration, dur_zh(duration))
    if not (30.0 <= duration <= 60.0):
        raise SystemExit(f"口播时长必须 30–60 秒，现在 {dur_zh(duration)}")
    for row in cues:
    ...
    jobs = [
        ("S02", render_b_hook, "B-钩子没咬住.mp4"),
        ("S04", render_b_slide, "B-人滑走.mp4"),
        ("S06", render_b_wait, "B-问号留空.mp4"),
    ]
    ...
    for sid, fn, fname in jobs:
        ...
        prev = by_file.get(fname)
        by_file[fname] = (fn, d if prev is None else max(prev[1], d))
    for fname, (fn, d) in by_file.items():
