#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""成片 57：一张对照卡只打一个点。云端 A-roll + B-roll，不是短剧。"""
...
ASSET_06 = CLOUD / "06" / "assets"
ASSET_25 = CLOUD / "25" / "assets"
...
def align_by_words(phrases: list[str], bounds: list[dict], duration: float) -> list[tuple[float, float, str]] | None:
    words = [b for b in bounds if b.get("type") == "WordBoundary" and b.get("text")]
    if not words:
        return None
    stream: list[tuple[str, float, float]] = []
    for w in words:
        st = ticks_to_sec(w.get("offset", 0))
        en = st + ticks_to_sec(w.get("duration", 0))
    ...
    for phrase in phrases:
        ...
        if start_t is None:
            start_t = cues[-1][1] if cues else 0.0
        if end_t is None:
            end_t = min(duration, start_t + 1.2)
        cues.append((start_t, end_t, phrase))
...
def make_voiceover() -> tuple[float, list[tuple[float, float, str]]]:
    ...
    try:
        ...
        aligned = align_by_words(phrases, bounds, probe_dur(wav))
        print("TTS ok", wav, "word cues", None if aligned is None else len(aligned))
...
def render_b_three(duration: float) -> list[Image.Image]:
    ...
    for i in range(n):
        ...
        tag(d, t, "密度 · 不是字多")
        a0 = appear(t, 0.02)
        d.text((W // 2, 236 + int(lerp(16, 0, a0))), "不是字多", font=font(58), fill=mix(BG, WHITE, a0), anchor="mm")
        d.text((W // 2, 310), "是一屏同时打三枪", font=font(34), fill=mix(BG, MUTED, appear(t, 0.16)), anchor="mm")
        if t < duration * 0.48:
            labels = ["建议一", "建议二", "建议三"]
            for k, name in enumerate(labels):
                a = appear(t, 0.22 + k * 0.16, 0.18)
                if a < 0.04:
                    continue
                y = 380 + k * 240
                rounded(d, (120, y, 960, y + 210), 32, mix(BG, CARD, a))
                d.text((W // 2, y + 105), name, font=font(52), fill=mix(CARD, WHITE, a), anchor="mm")
        else:
            a = appear(t, duration * 0.48, 0.22)
            d.text((W // 2, 400), "不要这样", font=font(34), fill=mix(BG, RED, a), anchor="mm")
            rounded(d, (110, 470, 970, 1180), 36, mix(BG, CARD, a))
            d.text((W // 2, 700), "三枪齐发", font=font(64), fill=mix(CARD, WHITE, a), anchor="mm")
            d.text((W // 2, 860), "一枪都没进", font=font(56), fill=mix(CARD, RED, a), anchor="mm")
            strike_line(d, (220, 640, 860, 760), appear(t, duration * 0.48 + 0.36, 0.24), mix(CARD, RED, a))
        out.append(img)
    return out
...
def render_b_sides(duration: float) -> list[Image.Image]:
    ...
    for i in range(n):
        ...
        d = ImageDraw.Draw(img)
        tag(d, t, "对照 · 左右一眼")
        a = appear(t, 0.02, 0.30)
        left = int(lerp(-420, 70, a))
        right = int(lerp(W + 40, 560, a))
        rounded(d, (left, 300, left + 450, 1280), 36, mix(BG, CARD, a))
        rounded(d, (right, 300, right + 450, 1280), 36, mix(BG, CARD, a))
        d.text((left + 225, 420), "左边", font=font(34), fill=mix(BG, RED, a), anchor="mm")
        d.text((left + 225, 620), "错法", font=font(64), fill=mix(BG, WHITE, a), anchor="mm")
        d.text((left + 225, 980), "一屏三件事", font=font(34), fill=mix(BG, MUTED, a), anchor="mm")
        d.text((right + 225, 420), "右边", font=font(34), fill=mix(BG, MINT, a), anchor="mm")
        d.text((right + 225, 620), "做法", font=font(64), fill=mix(BG, YELLOW, a), anchor="mm")
        d.text((right + 225, 980), "一对反差", font=font(34), fill=mix(BG, WHITE, a), anchor="mm")
        if a > 0.7:
            d.ellipse((W // 2 - 46, 740, W // 2 + 46, 832), fill=(18, 22, 28))
            d.text((W // 2, 786), "切", font=font(36), fill=YELLOW, anchor="mm")
        out.append(img)
    return out
...
def render_b_one(duration: float) -> list[Image.Image]:
    ...
    for i in range(n):
        ...
        tag(d, t, "规则 · 一张一点")
        a0 = appear(t, 0.02)
        d.text((W // 2, 280 + int(lerp(20, 0, a0))), "一张对照卡", font=font(58), fill=mix(BG, WHITE, a0), anchor="mm")
        a1 = appear(t, 0.28)
        if a1 > 0.04:
            y = 430 + int(lerp(26, 0, a1))
            rounded(d, (90, y, 990, y + 620), 36, mix(BG, CARD, a1))
            d.text((W // 2, y + 200), "只打", font=font(40), fill=mix(CARD, MUTED, a1), anchor="mm")
            d.text((W // 2, y + 360), "一个点", font=font(76), fill=mix(CARD, YELLOW, a1), anchor="mm")
        punch = appear(t, 1.20, 0.24)
        if punch > 0.04:
            y = 1180 + int(lerp(20, 0, punch))
            rounded(d, (160, y, 920, y + 180), 28, mix(BG, (18, 42, 36), punch))
            d.text((W // 2, y + 90), "一张卡 · 一个点", font=font(44), fill=mix(BG, MINT, punch), anchor="mm")
        out.append(img)
    return out
...
def render_b_contrast(duration: float) -> list[Image.Image]:
    ...
    for i in range(n):
        ...
        if t < duration * 0.38:
            a = appear(t, 0.02)
            d.text((W // 2, 250 + int(lerp(18, 0, a))), "信息图不是目录墙", font=font(50), fill=mix(BG, WHITE, a), anchor="mm")
            items = ["要点 1", "要点 2", "要点 3", "要点 4", "要点 5"]
            for k, name in enumerate(items):
                aa = appear(t, 0.18 + k * 0.10, 0.16)
                if aa < 0.04:
                    continue
                y = 360 + k * 200
                rounded(d, (150, y, 930, y + 168), 26, mix(BG, CARD, aa))
                d.text((W // 2, y + 84), name, font=font(46), fill=mix(CARD, MUTED, aa), anchor="mm")
        else:
            a = appear(t, duration * 0.38, 0.24)
            d.text((W // 2, 250), "他要记住的", font=font(34), fill=mix(BG, MUTED, a), anchor="mm")
            d.text((W // 2, 330), "不是五条建议", font=font(52), fill=mix(BG, WHITE, a), anchor="mm")
            rounded(d, (90, 430, 990, 1280), 36, mix(BG, CARD, a))
            d.text((W // 2, 620), "是这一对", font=font(36), fill=mix(CARD, MUTED, a), anchor="mm")
            d.text((W // 2, 780), "对 / 错", font=font(72), fill=mix(CARD, YELLOW, a), anchor="mm")
            d.text((W // 2, 1020), "一张卡，一个反差", font=font(44), fill=mix(CARD, MINT, a), anchor="mm")
        out.append(img)
    return out
...
def build_timeline(cues: list[tuple[float, float, str]], duration: float) -> dict:
    ...
    if len(cues) != 14:
        raise SystemExit(f"need 14 phrases, got {len(cues)}")
    p = cues
    cut_three = max(p[1][0] + 0.55, p[2][0] - LEAD)
    cut_sides = max(p[4][0] + 0.50, p[5][0] - LEAD)
    cut_one = max(p[6][1] - 0.02, p[7][0] - LEAD)
    cut_contrast = max(p[7][1] - 0.02, p[8][0] - LEAD)
    cut_close = max(p[11][1], p[12][0] - 0.04)
    if cut_three <= p[0][1] + 0.28:
        cut_three = p[2][0]
    if cut_sides <= p[3][1] + 0.28:
        cut_sides = p[5][0]
    if cut_one <= cut_sides + 0.36:
        cut_one = p[7][0]
    if cut_contrast <= cut_one + 0.36:
        cut_contrast = p[8][0]
    if cut_close <= cut_contrast + 0.36:
        cut_close = p[12][0]

    shots = [
        {"id": "S01a", "kind": "A", "start": 0.0, "end": p[0][1], "src": "assets/V-挥手.mp4", "line": p[0][2], "close": True},
        {"id": "S01b", "kind": "A", "start": p[0][1], "end": cut_three, "src": "assets/V-正对讲.mp4", "line": p[1][2]},
        {"id": "S02", "kind": "B", "start": cut_three, "end": p[3][1], "src": "broll/B-一屏三枪.mp4", "line": f"{p[2][2]} {p[3][2]}", "broll": "three"},
        {"id": "S03a", "kind": "A", "start": p[3][1], "end": cut_sides, "src": "assets/V-指向.mp4", "line": p[4][2]},
        {"id": "S03b", "kind": "B", "start": cut_sides, "end": cut_one, "src": "broll/B-左右对照.mp4", "line": f"{p[5][2]} {p[6][2]}", "broll": "sides"},
        {"id": "S04", "kind": "B", "start": cut_one, "end": cut_contrast, "src": "broll/B-一张一点.mp4", "line": p[7][2], "broll": "one"},
        {"id": "S05", "kind": "B", "start": cut_contrast, "end": cut_close, "src": "broll/B-反差.mp4", "line": f"{p[8][2]} {p[11][2]}", "broll": "contrast"},
        {"id": "S06", "kind": "A", "start": cut_close, "end": duration, "src": "assets/V-点赞.mp4", "line": f"{p[12][2]} {p[13][2]}"},
    ]
    ...
    a_caps = [
        ...
        {"start": round(p[1][0], 3), "end": round(min(p[1][1], cut_three), 3), "lines": ["知识口播最容易胖的"]},
        {"start": round(p[4][0], 3), "end": round(min(p[4][1], cut_sides), 3), "lines": split_caption(p[4][2])},
        {"start": round(p[12][0], 3), "end": round(p[12][1], 3), "lines": split_caption(p[12][2])},
        {"start": round(p[13][0], 3), "end": round(duration, 3), "lines": ["再写金句"]},
    ]
    a_caps = [c for c in a_caps if c["end"] > c["start"] + 0.08]

    shutters = [
        {"start": round(p[0][1], 3), "color": list(CREAM)},
        {"start": round(cut_three, 3), "color": list(MINT)},
        {"start": round(p[3][1], 3), "color": list(CREAM)},
        {"start": round(cut_sides, 3), "color": list(MINT)},
        {"start": round(cut_one, 3), "color": list(YELLOW)},
        {"start": round(cut_contrast, 3), "color": list(MINT)},
        {"start": round(cut_close, 3), "color": list(YELLOW)},
    ]
    eyebrows = [
        {"start": 0.0, "end": round(p[0][1], 3), "text": "A-ROLL / 1a"},
        {"start": round(p[0][1], 3), "end": round(cut_three, 3), "text": "A-ROLL / 1b"},
        {"start": round(p[3][1], 3), "end": round(cut_sides, 3), "text": "A-ROLL / 03"},
        {"start": round(cut_close, 3), "end": round(duration, 3), "text": "A-ROLL / 06"},
    ]
    data = {
...
def make_cover() -> Path:
    ...
    d.text((W // 2, 160), "一张对照卡", font=font(48), fill=WHITE, anchor="mm")
    d.text((W // 2, 250), "只打一个点", font=font(64), fill=YELLOW, anchor="mm")
...
def copy_assets() -> None:
    ...
    mapping = {
        "V-挥手.mp4": ASSET_06 / "V-挥手.mp4",
        "V-指向.mp4": ASSET_06 / "V-指向.mp4",
        "V-点赞.mp4": ASSET_06 / "V-点赞.mp4",
        "V-摊手.mp4": ASSET_06 / "V-摊手.mp4",
        "A-角色-小灯-摊手.jpg": ASSET_06 / "A-角色-小灯-摊手.jpg",
        "V-正对讲.mp4": ASSET_25 / "V-正对讲.mp4",
    }
    for name, src in mapping.items():
        if not src.exists():
            if name == "V-正对讲.mp4":
                src = ASSET_06 / "V-摊手.mp4"
            else:
                raise FileNotFoundError(src)
        target = dest / name
        if not target.exists() or target.stat().st_size != src.stat().st_size:
            shutil.copy2(src, target)
    still = dest / "A-角色-小灯-指向.jpg"
    if not still.exists():
        run([
            "ffmpeg", "-y", "-i", str(dest / "V-指向.mp4"),
            "-ss", "0.40", "-frames:v", "1", "-q:v", "2", str(still),
        ])
...
def main() -> None:
    ...
    bmap = {
        "S02": (render_b_three, ROOT / "broll" / "B-一屏三枪.mp4"),
        "S03b": (render_b_sides, ROOT / "broll" / "B-左右对照.mp4"),
        "S04": (render_b_one, ROOT / "broll" / "B-一张一点.mp4"),
        "S05": (render_b_contrast, ROOT / "broll" / "B-反差.mp4"),
    }
    for sid, (fn, dest) in bmap.items():
        shot = next(s for s in data["shots"] if s["id"] == sid)
        dur = max(2.2, float(shot["end"]) - float(shot["start"]))
        print("render", dest.name, dur)
        frames_to_mp4(fn(dur + 0.12), dest)
