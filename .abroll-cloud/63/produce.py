#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Topic 63：排期只有乐观数。工厂 63_排期只有乐观数。云端 NEW A-roll + B-roll。

普通短视频 / 知识口播。草稿只写 .abroll-cloud/63/，成品中转 成片/63-排期只有乐观数.mp4。
...
def punch_bar(draw: ImageDraw.ImageDraw, t: float, start: float, text: str, good: bool = False) -> None:
    a = appear(t, start, 0.22)
    if a <= 0.04:
        return
    y = 1040 + bob(t, 7, 0.8)
    fill = (18, 42, 36) if good else (42, 24, 22)
    ink = MINT if good else YELLOW
    rounded(draw, (120, y, 960, y + 170), 28, mix(BG, fill, a))
    draw.text((W // 2, y + 85), text, font=font(46), fill=mix(BG, ink, a), anchor="mm")


def draw_b_only_one(t: float) -> Image.Image:
    img = new_bg(t)
    d = ImageDraw.Draw(img)
    tag(d, t, "错 · 只有一格")
    d.text((W // 2, 236 + bob(t, 6, 0.5)), "只写最快那天", font=font(54), fill=mix(BG, WHITE, appear(t, 0.02)), anchor="mm")
    a1 = appear(t, 0.12)
    y = 330 + bob(t, 8, 0.55)
    rounded(d, (90, y, 990, y + 520), 36, mix(BG, CARD, a1))
    d.text((W // 2, y + 70), "排期表", font=font(30), fill=mix(CARD, MUTED, a1), anchor="mm")
    cells = [("一", False), ("二", False), ("三", True), ("四", False), ("五", False)]
    for i, (lab, hot) in enumerate(cells):
        x = 140 + i * 160
        cy = y + 200
        col = YELLOW if hot else (48, 52, 62)
        rounded(d, (x, cy, x + 140, cy + 160), 22, mix(CARD, col, a1))
        d.text((x + 70, cy + 50), lab, font=font(28), fill=mix(col, MUTED if not hot else INK, a1), anchor="mm")
        d.text((x + 70, cy + 110), "最快" if hot else "空", font=font(32), fill=mix(col, INK if hot else MUTED, a1), anchor="mm")
    contrast = appear(t, 0.55)
    y2 = 900 + bob(t, 6, 0.7)
    rounded(d, (90, y2, 520, y2 + 120), 24, mix(BG, (42, 32, 18), contrast))
    d.text((305, y2 + 60), "一切顺利", font=font(36), fill=mix(CARD, YELLOW, contrast), anchor="mm")
    rounded(d, (560, y2, 990, y2 + 120), 24, mix(BG, (42, 22, 22), contrast))
    d.text((775, y2 + 60), "不是能守住", font=font(36), fill=mix(CARD, RED, contrast), anchor="mm")
    punch_bar(d, t, 1.00, "顺利不是能守住")
    return img


def draw_b_three_hits(t: float) -> Image.Image:
    img = new_bg(t)
    d = ImageDraw.Draw(img)
    tag(d, t, "错 · 砸掉")
    d.text((W // 2, 236 + bob(t, 6, 0.5)), "三件事一来", font=font(54), fill=mix(BG, WHITE, appear(t, 0.02)), anchor="mm")
    rows = [
        (0.12, "接口晚一天", "联调对不上"),
        (0.32, "评审多一轮", "那天往后挪"),
        (0.52, "人一请假", "格子空了"),
    ...
    for i, (ts, head, body) in enumerate(rows):
        ...
        if a < 0.04:
            continue
        y = 330 + i * 168 + bob(t + i * 0.15, 7, 0.6)
        rounded(d, (90, y, 990, y + 148), 26, mix(BG, CARD, a))
        d.ellipse((128, y + 36, 220, y + 128), fill=mix(CARD, RED, a))
        d.text((174, y + 82), str(i + 1), font=font(36), fill=mix(RED, WHITE, a), anchor="mm")
        d.text((248, y + 52), head, font=font(40), fill=mix(CARD, YELLOW, a), anchor="lm")
        d.text((248, y + 112), body, font=font(28), fill=mix(CARD, MUTED, a), anchor="lm")
        strike = appear(t, ts + 0.35, 0.22)
        if strike > 0.04:
            strike_line(d, (240, y + 20, 960, y + 90), strike, mix(CARD, RED, strike))
    punch_bar(d, t, 1.00, "日子还在，活对不上")
    return img


def draw_b_umbrella(t: float) -> Image.Image:
    img = new_bg(t)
    d = ImageDraw.Draw(img)
    tag(d, t, "错 · 不带伞")
    d.text((W // 2, 236 + bob(t, 6, 0.5)), "出门不带伞", font=font(56), fill=mix(BG, WHITE, appear(t, 0.02)), anchor="mm")
    a1 = appear(t, 0.14)
    y = 340 + bob(t, 8, 0.55)
    rounded(d, (90, y, 520, y + 420), 32, mix(BG, CARD, a1))
    d.text((305, y + 70), "乐观数", font=font(30), fill=mix(CARD, MUTED, a1), anchor="mm")
    sun = 28 + int(8 * math.sin(t * 3.0))
    d.ellipse((305 - sun, y + 170 - sun, 305 + sun, y + 170 + sun), fill=mix(CARD, YELLOW, a1))
    d.text((305, y + 300), "当晴天出发", font=font(36), fill=mix(CARD, YELLOW, a1), anchor="mm")
    rain_a = appear(t, 0.36)
    rounded(d, (560, y + bob(t + 0.2, 7, 0.6), 990, y + 420), 32, mix(BG, (28, 34, 48), rain_a))
    d.text((775, y + 70), "真实天气", font=font(30), fill=mix(CARD, MUTED, rain_a), anchor="mm")
    for i in range(5):
        x = 640 + i * 56
        drop_y = y + 150 + int((t * 90 + i * 37) % 160)
        d.ellipse((x, drop_y, x + 16, drop_y + 28), fill=mix(CARD, RAIN, rain_a))
    d.text((775, y + 340), "没带伞", font=font(40), fill=mix(CARD, RED, rain_a), anchor="mm")
    punch_bar(d, t, 0.95, "把晴天当成了天气")
    return img


def draw_b_wait_slip(t: float) -> Image.Image:
    img = new_bg(t)
    d = ImageDraw.Draw(img)
    tag(d, t, "错 · 等人")
    d.text((W // 2, 236 + bob(t, 6, 0.5)), "别人按那天来等", font=font(50), fill=mix(BG, WHITE, appear(t, 0.02)), anchor="mm")
    a1 = appear(t, 0.14)
    y = 340 + bob(t, 8, 0.55)
    rounded(d, (90, y, 990, y + 280), 32, mix(BG, CARD, a1))
    d.text((W // 2, y + 80), "对外承诺", font=font(32), fill=mix(CARD, MUTED, a1), anchor="mm")
    d.text((W // 2, y + 170), "周三交", font=font(72), fill=mix(CARD, YELLOW, a1), anchor="mm")
    slip = appear(t, 0.50, 0.35)
    y2 = 680 + bob(t, 7, 0.65)
    rounded(d, (90, y2, 990, y2 + 240), 32, mix(BG, (42, 24, 22), slip))
    d.text((W // 2, y2 + 80), "等来的是", font=font(32), fill=mix(CARD, MUTED, slip), anchor="mm")
    d.text((W // 2, y2 + 160), "延期", font=font(64), fill=mix(CARD, RED, slip), anchor="mm")
    stamp = appear(t, 0.72, 0.20)
    if stamp > 0.04:
        d.ellipse((760, y2 + 40, 940, y2 + 220), outline=mix(CARD, RED, stamp), width=10)
    punch_bar(d, t, 1.00, "迟到的是你")
    return img


def draw_b_two_boxes(t: float) -> Image.Image:
    img = new_bg(t)
    d = ImageDraw.Draw(img)
    tag(d, t, "对 · 两格")
    d.text((W // 2, 236 + bob(t, 6, 0.5)), "排期写两格", font=font(56), fill=mix(BG, WHITE, appear(t, 0.02)), anchor="mm")
    a1 = appear(t, 0.14)
    y = 340 + bob(t, 7, 0.5)
    rounded(d, (80, y, 520, y + 460), 32, mix(BG, CARD, a1))
    d.text((300, y + 80), "一格", font=font(30), fill=mix(CARD, MUTED, a1), anchor="mm")
    d.text((300, y + 190), "顺利能到", font=font(46), fill=mix(CARD, YELLOW, a1), anchor="mm")
    d.text((300, y + 300), "内部最快", font=font(32), fill=mix(CARD, MUTED, a1), anchor="mm")
    good = appear(t, 0.32)
    rounded(d, (560, y + bob(t + 0.2, 7, 0.55), 1000, y + 460), 32, mix(BG, (22, 40, 36), good))
    d.text((780, y + 80), "一格", font=font(30), fill=mix(CARD, MUTED, good), anchor="mm")
    d.text((780, y + 190), "出岔还能到", font=font(44), fill=mix(CARD, MINT, good), anchor="mm")
    d.text((780, y + 300), "对外能守住", font=font(32), fill=mix(CARD, WHITE, good), anchor="mm")
    check_badge(d, 780, y + 390, appear(t, 0.80, 0.18))
    punch_bar(d, t, 1.00, "出岔还能到", good=True)
    return img


def draw_b_inside_out(t: float) -> Image.Image:
    img = new_bg(t)
    d = ImageDraw.Draw(img)
    tag(d, t, "对 · 对外")
    d.text((W // 2, 236 + bob(t, 6, 0.5)), "内部最快别外报", font=font(50), fill=mix(BG, WHITE, appear(t, 0.02)), anchor="mm")
    a1 = appear(t, 0.14)
    y = 340 + bob(t, 8, 0.55)
    rounded(d, (90, y, 990, y + 220), 32, mix(BG, CARD, a1))
    d.text((W // 2, y + 70), "内部", font=font(30), fill=mix(CARD, MUTED, a1), anchor="mm")
    d.text((W // 2, y + 150), "最快能到", font=font(52), fill=mix(CARD, YELLOW, a1), anchor="mm")
    fly = appear(t, 0.36, 0.55)
    y2 = int(lerp(720, 640, fly)) + bob(t, 6, 0.6)
    rounded(d, (90, y2, 990, y2 + 240), 32, mix(BG, (22, 40, 36), appear(t, 0.32)))
    d.text((W // 2, y2 + 70), "对外", font=font(30), fill=mix(CARD, MUTED, 1), anchor="mm")
    d.text((W // 2, y2 + 150), "只报能守住", font=font(52), fill=mix(CARD, MINT, 1), anchor="mm")
    check_badge(d, 930, y2 + 70, appear(t, 0.80, 0.18))
    strike = appear(t, 0.55, 0.28)
    if strike > 0.04:
        strike_line(d, (220, y + 110, 860, y + 190), strike, mix(CARD, RED, strike))
    punch_bar(d, t, 1.00, "别写成对外截止", good=True)
    return img


BROLL_DRAW = {
    "only_one": draw_b_only_one,
    "three_hits": draw_b_three_hits,
    "umbrella": draw_b_umbrella,
    "wait_slip": draw_b_wait_slip,
    "two_boxes": draw_b_two_boxes,
    "inside_out": draw_b_inside_out,
}
...
def build_timeline(cues: list[tuple[float, float, str]], duration: float) -> dict:
    ...
    data = {
        ...
        "b_lead_s": LEAD,
        "video_type": "普通短视频",
        "source_note": "工厂 63_排期只有乐观数 / 云端 NEW",
...
def make_cover() -> Path:
    ...
    d.text((W // 2, 160), "排期只有", font=font(52), fill=WHITE, anchor="mm")
    d.text((W // 2, 250), "乐观数", font=font(72), fill=YELLOW, anchor="mm")
...
1080×1920，24 fps，H.264 + AAC 44100 stereo，约 {zh_sec(duration)}（目标四十到五十秒）。镜头 {len(data["shots"])} 条，时间轴闭合。
"""
...
def patch_index(duration: float) -> None:
    ...
    if delivery.exists():
        ...
        row = f"| 63 | `{STAGED_NAME}` | {zh_sec(duration)} | 已核验 |"
        if "`63-排期只有乐观数.mp4`" in dtext:
            lines = []
            for raw in dtext.splitlines():
                if raw.startswith("| 63 |") or (raw.startswith("|") and "`63-排期只有乐观数.mp4`" in raw):
                    lines.append(row)
                else:
                    lines.append(raw)
            dtext = "\n".join(lines)
            if not dtext.endswith("\n"):
                dtext += "\n"
            delivery.write_text(dtext, encoding="utf-8")
        elif "## 本轮 61+（核验中）" in dtext:
            if dtext.rstrip().endswith("|"):
                delivery.write_text(dtext.rstrip() + "\n" + row + "\n", encoding="utf-8")
...
def main() -> None:
    ...
    if not (TARGET_LO <= duration <= TARGET_HI):
        print("WARN voiceover outside target window", zh_sec(duration))
    make_bgm(duration)
