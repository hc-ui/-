...
def patch_topics(duration: float) -> None:
    ...
    old = "| 73 | 未完成 Python | `73-int-input会炸先包一层.mp4` | 未拍 |"
    claimed_row = "| 73 | 未完成 Python | `73-int-input先包一层.mp4` | 已认领 `.abroll-cloud/73/` |"
    new = f"| 73 | 未完成 Python | `{STAGED_NAME}` | 已核验 {kit.zh_sec(duration)} |"
    if old in text:
        text = text.replace(old, new)
    elif claimed_row in text:
        text = text.replace(claimed_row, new)
    elif f"`{STAGED_NAME}`" in text and "| 73 |" in text:
        lines = []
        for raw in text.splitlines():
            if raw.startswith("| 73 |") and STAGED_NAME in raw:
                lines.append(new)
            else:
                lines.append(raw)
        text = "\n".join(lines)
    ...
    elif verified not in text and "## 73. int(input) 会炸，先包一层" in text:
        text = text.replace(
