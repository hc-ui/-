#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Topic 81：请假带两方案。备忘录第三节第三步。云端 NEW A-roll + B-roll。

普通短视频 / 知识口播。草稿只写 .abroll-cloud/81/，成品中转 成片/81-请假带两方案.mp4。
禁止 C:\\ D:\\ G:\\，禁止 Drive 上传，不走 drama-pipeline。
目标四十到五十秒。不定格、不慢放注水。时长用中文「秒」。
不讲成片 37 先摸惯例、工期大饼、真名、话术全文。
"""
...
def draw_slip(draw: ImageDraw.ImageDraw, x0: int, y0: int, w: int, h: int, a: float, title: str, blank: bool) -> None:
    if a <= 0.04:
        return
    paper = mix(BG, PAPER if not blank else (48, 50, 56), a)
    rounded(draw, (x0, y0, x0 + w, y0 + h), 22, paper)
    draw.rectangle((x0 + 18, y0 + 16, x0 + w - 18, y0 + 28), fill=mix(paper, RED if blank else (180, 64, 56), a))
    draw.text((x0 + w // 2, y0 + 64), title, font=font(28), fill=mix(paper, MUTED if blank else INK, a), anchor="mm")
    if blank:
        draw.text((x0 + w // 2, y0 + h // 2 + 20), "空白", font=font(48), fill=mix(paper, RED, a), anchor="mm")
    else:
        for i, line in enumerate(("冲突", "课表", "补做")):
            yy = y0 + 110 + i * 52
            draw.rectangle((x0 + 40, yy, x0 + w - 40, yy + 12), fill=mix(paper, INK, a))
            draw.text((x0 + 40, yy - 24), line, font=font(22), fill=mix(paper, INK, a), anchor="lm")


def draw_ppt(draw: ImageDraw.ImageDraw, x0: int, y0: int, w: int, h: int, a: float) -> None:
    if a <= 0.04:
        return
    cover = mix(BG, (28, 46, 58), a)
    rounded(draw, (x0 + 16, y0 + 18, x0 + w + 16, y0 + h + 18), 18, mix(BG, (18, 28, 36), a))
    rounded(draw, (x0, y0, x0 + w, y0 + h), 18, cover)
    draw.rectangle((x0 + 28, y0 + 36, x0 + w - 28, y0 + 52), fill=mix(cover, MINT, a))
    draw.text((x0 + w // 2, y0 + 96), "汇报稿", font=font(32), fill=mix(cover, WHITE, a), anchor="mm")
    for i in range(3):
        yy = y0 + 140 + i * 36
        draw.rectangle((x0 + 40, yy, x0 + w - 40, yy + 10), fill=mix(cover, MUTED, a))


def draw_b_leave_ppt(t: float) -> Image.Image:
    img = new_bg(t)
    d = ImageDraw.Draw(img)
    tag(d, t, "对 · 随假钉稿")
    d.text((W // 2, 236 + bob(t, 7, 0.5)), "假条下钉着稿", font=font(52), fill=mix(BG, WHITE, appear(t, 0.02)), anchor="mm")
    a1 = appear(t, 0.12)
    draw_ppt(d, 170, 340 + bob(t, 6, 0.45), 740, 320, a1)
    pin = appear(t, 0.36, 0.28)
    slip_y = 620 + bob(t + 0.2, 8, 0.55)
    draw_slip(d, 150, slip_y, 780, 360, max(a1, pin), "请假条", False)
    if pin > 0.04:
        cx, cy = 540, slip_y + 8
        d.ellipse((cx - 18, cy - 18, cx + 18, cy + 18), fill=mix(CARD, YELLOW, pin))
        d.line([(cx, cy + 16), (cx, cy + 70)], fill=mix(CARD, YELLOW, pin), width=8)
    punch = appear(t, 1.00, 0.22)
    if punch > 0.04:
        y = 1040 + bob(t, 7, 0.8)
        rounded(d, (120, y, 960, y + 170), 28, mix(BG, (42, 32, 16), punch))
        d.text((W // 2, y + 85), "先交稿", font=font(52), fill=mix(BG, YELLOW, punch), anchor="mm")
    return img


def draw_b_two_plans(t: float) -> Image.Image:
    img = new_bg(t)
    d = ImageDraw.Draw(img)
    tag(d, t, "对 · 二选一")
    d.text((W // 2, 236 + bob(t, 6, 0.5)), "两张卡并列", font=font(54), fill=mix(BG, WHITE, appear(t, 0.02)), anchor="mm")
    cards = [
        (0.12, 70, "方案一", "开头先汇报", "再去上课", YELLOW),
        (0.36, 560, "方案二", "课后补做", "或下次补", MINT),
    ]
    for ts, x, head, line1, line2, color in cards:
        a = appear(t, ts, 0.22)
        if a < 0.04:
            continue
        y = 330 + bob(t + ts, 8, 0.55)
        rounded(d, (x, y, x + 450, y + 520), 32, mix(BG, CARD, a))
        d.rounded_rectangle((x + 28, y + 28, x + 200, y + 88), 16, fill=mix(CARD, color, a))
        d.text((x + 114, y + 58), head, font=font(28), fill=mix(color, INK, a), anchor="mm")
        d.text((x + 225, y + 200), line1, font=font(40), fill=mix(CARD, WHITE, a), anchor="mm")
        d.text((x + 225, y + 280), line2, font=font(36), fill=mix(CARD, color, a), anchor="mm")
        box = (x + 170, y + 360, x + 280, y + 470)
        rounded(d, box, 18, mix(CARD, (18, 22, 28), a))
        check_badge(d, x + 225, y + 415, appear(t, ts + 0.55, 0.18) * 0.35)
    punch = appear(t, 1.10, 0.20)
    if punch > 0.04:
        y = 920 + bob(t, 7, 0.8)
        rounded(d, (140, y, 940, y + 160), 26, mix(BG, (18, 42, 36), punch))
        d.text((W // 2, y + 80), "让他勾一张", font=font(50), fill=mix(BG, MINT, punch), anchor="mm")
    return img


def draw_b_reason_vs_plan(t: float) -> Image.Image:
    img = new_bg(t)
    d = ImageDraw.Draw(img)
    tag(d, t, "错 · 主观理由")
    d.text((W // 2, 236 + bob(t, 6, 0.5)), "理由对不上刚性", font=font(48), fill=mix(BG, WHITE, appear(t, 0.02)), anchor="mm")
    a1 = appear(t, 0.14)
    y = 340 + bob(t, 8, 0.55)
    rounded(d, (70, y, 510, y + 420), 32, mix(BG, (42, 24, 22), a1))
    d.text((290, y + 90), "主观理由", font=font(30), fill=mix(CARD, MUTED, a1), anchor="mm")
    d.text((290, y + 190), "家里有事", font=font(40), fill=mix(CARD, RED, a1), anchor="mm")
    strike_line(d, (110, y + 160, 470, y + 220), appear(t, 0.40, 0.4), mix(CARD, RED, 1))
    draw_x(d, 290, y + 310, appear(t, 0.48, 0.2), 48)
    a2 = appear(t, 0.28)
    rounded(d, (570, y + bob(t + 0.2, 7, 0.5), 1010, y + 420), 32, mix(BG, (22, 40, 36), a2))
    d.text((790, y + 90), "培养方案", font=font(30), fill=mix(CARD, MUTED, a2), anchor="mm")
    d.text((790, y + 190), "必修考勤", font=font(40), fill=mix(CARD, MINT, a2), anchor="mm")
    d.text((790, y + 270), "刚性冲突", font=font(32), fill=mix(CARD, WHITE, a2), anchor="mm")
    check_badge(d, 790, y + 350, appear(t, 0.70, 0.18))
    punch = appear(t, 1.05, 0.22)
    if punch > 0.04:
        y = 820 + bob(t, 7, 0.8)
        rounded(d, (120, y, 960, y + 180), 28, mix(BG, (18, 42, 36), punch))
        d.text((W // 2, y + 90), "把冲突写清楚", font=font(48), fill=mix(BG, MINT, punch), anchor="mm")
    return img


def draw_b_empty_throw(t: float) -> Image.Image:
    img = new_bg(t)
    d = ImageDraw.Draw(img)
    tag(d, t, "错 · 空手假条")
    d.text((W // 2, 236 + bob(t, 6, 0.5)), "空手等于扔回去", font=font(48), fill=mix(BG, WHITE, appear(t, 0.02)), anchor="mm")
    fly = appear(t, 0.14, 0.5)
    dx = int(lerp(0, 80, fly) * math.sin(t * 1.2))
    draw_slip(d, 160 + dx, 330 + bob(t, 10, 0.7), 340, 380, appear(t, 0.10), "假条", True)
    ball = appear(t, 0.36)
    cx = int(lerp(720, 860, 0.5 + 0.5 * math.sin(t * 2.2)))
    cy = 520 + bob(t + 0.3, 14, 0.9)
    d.ellipse((cx - 110, cy - 110, cx + 110, cy + 110), fill=mix(BG, (42, 28, 18), ball))
    d.text((cx, cy), "决定", font=font(44), fill=mix(CARD, YELLOW, ball), anchor="mm")
    d.text((cx, cy + 160), "扔回给他", font=font(32), fill=mix(BG, RED, ball), anchor="mm")
    punch = appear(t, 1.00, 0.22)
    if punch > 0.04:
        y = 900 + bob(t, 7, 0.8)
        rounded(d, (120, y, 960, y + 180), 28, mix(BG, (42, 24, 22), punch))
        d.text((W // 2, y + 90), "等于没请", font=font(52), fill=mix(BG, YELLOW, punch), anchor="mm")
    return img


def draw_b_pick_one(t: float) -> Image.Image:
    img = new_bg(t)
    d = ImageDraw.Draw(img)
    tag(d, t, "收 · 勾一张")
    d.text((W // 2, 236 + bob(t, 6, 0.5)), "他勾哪张按哪张", font=font(48), fill=mix(BG, WHITE, appear(t, 0.02)), anchor="mm")
    a1 = appear(t, 0.12)
    y = 330 + bob(t, 7, 0.5)
    rounded(d, (80, y, 500, y + 400), 32, mix(BG, CARD, a1))
    d.text((290, y + 80), "方案一", font=font(30), fill=mix(CARD, MUTED, a1), anchor="mm")
    d.text((290, y + 180), "先汇报", font=font(44), fill=mix(CARD, YELLOW, a1), anchor="mm")
    d.text((290, y + 270), "再上课", font=font(32), fill=mix(CARD, WHITE, a1), anchor="mm")
    pick = appear(t, 0.55, 0.22)
    check_badge(d, 290, y + 350, pick)
    a2 = appear(t, 0.22)
    rounded(d, (580, y + bob(t + 0.15, 6, 0.45), 1000, y + 400), 32, mix(BG, CARD, a2))
    d.text((790, y + 80), "方案二", font=font(30), fill=mix(CARD, MUTED, a2), anchor="mm")
    d.text((790, y + 180), "课后补", font=font(44), fill=mix(CARD, MINT, a2), anchor="mm")
    d.text((790, y + 270), "或下次", font=font(32), fill=mix(CARD, WHITE, a2), anchor="mm")
    punch = appear(t, 1.00, 0.22)
    if punch > 0.04:
        y = 800 + bob(t, 7, 0.8)
        rounded(d, (120, y, 960, y + 180), 28, mix(BG, (18, 42, 36), punch))
        d.text((W // 2, y + 90), "按勾的走", font=font(52), fill=mix(BG, MINT, punch), anchor="mm")
    return img


BROLL_DRAW = {
    "leave_ppt": draw_b_leave_ppt,
    "two_plans": draw_b_two_plans,
    "reason_vs_plan": draw_b_reason_vs_plan,
    "empty_throw": draw_b_empty_throw,
    "pick_one": draw_b_pick_one,
}
...
def build_timeline(cues: list[tuple[float, float, str]], duration: float) -> dict:
    ...
    data = {
        ...
        "b_lead_s": LEAD,
        "video_type": "普通短视频",
        "source_note": "topics-batch4.md #81 / 备忘录第三节第三步 / 云端 NEW",
    }
...
def make_cover() -> Path:
    ...
    d.text((W // 2, 160), "请假带", font=font(52), fill=WHITE, anchor="mm")
    d.text((W // 2, 250), "两方案", font=font(72), fill=YELLOW, anchor="mm")
...
def write_docs(duration: float, staged: Path, data: dict) -> None:
    ...
    status = {
        ...
        "distinct_from": "成片/37-组会撞课先摸惯例再书面报备.mp4",
        "cloud_only": True,
        "windows_paths": False,
        "drive_upload": False,
        "not": "drama-pipeline / 仙侠连载 / C:D:G: / Drive 上传 / 成片37先摸惯例 / 工期大饼 / 真名话术",
        "updated_at": now,
    }
...
| 号 | 成片 | 对照 37 | 新时长 | 草稿 |
|----|------|---------|--------|------|
| 81 | `{STAGED_NAME}` | 十三点二秒（先摸惯例） | {zh_sec(duration)} | `.abroll-cloud/81/` |
...
def patch_delivery(duration: float) -> None:
    ...
    row = (
        ...
        f"topic 81 `topics-batch4.md` 第 81 条；备忘录第三节第三步。成片 37 只打先摸惯例。云端 NEW |"
    )
    ...
    if token in text:
        ...
        if marker in text:
            ...
            if not text.endswith("\n"):
                text += "\n"
            text += row + "\n"
    plus_row = f"| 81 | `{STAGED_NAME}` | {zh_sec(duration)} | 已核验 |"
    if "| 81 |" in text:
        rows = []
        for raw in text.splitlines():
            if raw.startswith("| 81 |"):
                ...
                rows.append(raw)
        text = "\n".join(rows)
    elif "| 69 |" in text and plus_row not in text:
        lines = text.splitlines()
        out = []
        inserted = False
        for raw in lines:
            out.append(raw)
            if raw.startswith("| 69 |") and not inserted:
                out.append(plus_row)
                inserted = True
        text = "\n".join(out)
    if not text.endswith("\n"):
...
def qa(staged: Path, data: dict) -> dict:
    ...
    banned = ["先摸惯例", "私下问同门", "工期大饼", "下一期", "赵老师"]
