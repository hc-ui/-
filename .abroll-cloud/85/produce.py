#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Topic 85：毕业底线三件套。备忘录第一节 3。云端 NEW A-roll + B-roll。

普通短视频 / 知识口播。草稿只写 .abroll-cloud/85/，成品中转 成片/85-毕业底线三件套.mp4。
禁止 C:\\ D:\\ G:\\，禁止 Drive 上传，不走 drama-pipeline。
目标四十到五十秒。不定格、不慢放注水。时长用中文「秒」。
不讲成片 14 双线、15 生态位、16 切片、49 精读、50 绑定博士。
"""
...
def draw_b_not_top(t: float) -> Image.Image:
    img = new_bg(t)
    d = ImageDraw.Draw(img)
    tag(d, t, "错 · 追错线")
    d.text((W // 2, 236 + bob(t, 6, 0.5)), "别拿错线当底线", font=font(48), fill=mix(BG, WHITE, appear(t, 0.02)), anchor="mm")
    a1 = appear(t, 0.14)
    y = 340 + bob(t, 8, 0.55)
    rounded(d, (70, y, 510, y + 420), 32, mix(BG, (42, 24, 22), a1))
    d.text((290, y + 90), "博士线", font=font(30), fill=mix(CARD, MUTED, a1), anchor="mm")
    d.text((290, y + 190), "医学顶刊", font=font(40), fill=mix(CARD, RED, a1), anchor="mm")
    strike_line(d, (110, y + 160, 470, y + 220), appear(t, 0.40, 0.4), mix(CARD, RED, 1))
    draw_x(d, 290, y + 310, appear(t, 0.48, 0.2), 48)
    a2 = appear(t, 0.28)
    rounded(d, (570, y + bob(t + 0.2, 7, 0.5), 1010, y + 420), 32, mix(BG, (42, 26, 20), a2))
    d.text((790, y + 90), "实验室", font=font(30), fill=mix(CARD, MUTED, a2), anchor="mm")
    d.text((790, y + 190), "湿实验扛完", font=font(40), fill=mix(CARD, RED, a2), anchor="mm")
    strike_line(d, (610, y + 160, 970, y + 220), appear(t, 0.52, 0.4), mix(CARD, RED, 1))
    draw_x(d, 790, y + 310, appear(t, 0.60, 0.2), 48)
    punch = appear(t, 1.00, 0.22)
    if punch > 0.04:
        y = 820 + bob(t, 7, 0.8)
        rounded(d, (120, y, 960, y + 180), 28, mix(BG, (42, 32, 16), punch))
        d.text((W // 2, y + 90), "那不是底线", font=font(52), fill=mix(BG, YELLOW, punch), anchor="mm")
    return img


def draw_b_three_cards(t: float) -> Image.Image:
    img = new_bg(t)
    d = ImageDraw.Draw(img)
    tag(d, t, "对 · 三张卡")
    d.text((W // 2, 220 + bob(t, 6, 0.5)), "学院只认这三张", font=font(48), fill=mix(BG, WHITE, appear(t, 0.02)), anchor="mm")
    cards = [
        (0.10, 80, "01", "按时开题", "窗口内交", YELLOW),
        (0.32, 80, "02", "工程论文", "过盲审", MINT),
        (0.54, 80, "03", "挂名成果", "专利或论文", YELLOW),
    ]
    for i, (ts, x, num, title, sub, color) in enumerate(cards):
        a = appear(t, ts, 0.22)
        if a < 0.04:
            continue
        y = 300 + i * 280 + bob(t + ts, 6, 0.5)
        rounded(d, (x, y, 1000, y + 250), 28, mix(BG, CARD, a))
        d.rounded_rectangle((x + 28, y + 28, x + 150, y + 96), 16, fill=mix(CARD, color, a))
        d.text((x + 89, y + 62), num, font=font(32), fill=mix(color, INK, a), anchor="mm")
        d.text((x + 420, y + 90), title, font=font(48), fill=mix(CARD, WHITE, a), anchor="lm")
        d.text((x + 420, y + 170), sub, font=font(34), fill=mix(CARD, color, a), anchor="lm")
        if i == 0:
            check_badge(d, 900, y + 125, appear(t, 0.28, 0.18))
    return img


def draw_b_stamp_pass(t: float) -> Image.Image:
    img = new_bg(t)
    d = ImageDraw.Draw(img)
    tag(d, t, "对 · 签字放行")
    d.text((W // 2, 236 + bob(t, 6, 0.5)), "三件打满才签字", font=font(48), fill=mix(BG, WHITE, appear(t, 0.02)), anchor="mm")
    rows = [("开题", 0.14), ("盲审论文", 0.34), ("挂名", 0.54)]
    y0 = 340 + bob(t, 6, 0.45)
    for i, (label, ts) in enumerate(rows):
        a = appear(t, ts, 0.2)
        y = y0 + i * 150
        rounded(d, (120, y, 960, y + 128), 24, mix(BG, CARD, a))
        d.text((280, y + 64), label, font=font(40), fill=mix(CARD, WHITE, a), anchor="mm")
        check_badge(d, 820, y + 64, appear(t, ts + 0.22, 0.16))
    punch = appear(t, 1.05, 0.22)
    if punch > 0.04:
        cx, cy = 540, 920 + bob(t, 8, 0.7)
        d.ellipse((cx - 150, cy - 150, cx + 150, cy + 150), outline=mix(BG, MINT, punch), width=10)
        d.text((cx, cy), "放行", font=font(64), fill=mix(BG, MINT, punch), anchor="mm")
    return img


def draw_b_window_block(t: float) -> Image.Image:
    img = new_bg(t)
    d = ImageDraw.Draw(img)
    tag(d, t, "错 · 窗口过了")
    d.text((W // 2, 236 + bob(t, 6, 0.5)), "开题一拖全堵死", font=font(48), fill=mix(BG, WHITE, appear(t, 0.02)), anchor="mm")
    a1 = appear(t, 0.12)
    y = 340 + bob(t, 7, 0.5)
    rounded(d, (90, y, 990, y + 280), 28, mix(BG, (42, 28, 18), a1))
    d.text((540, y + 80), "开题窗口", font=font(32), fill=mix(CARD, MUTED, a1), anchor="mm")
    d.text((540, y + 170), "已过", font=font(64), fill=mix(CARD, RED, a1), anchor="mm")
    draw_x(d, 900, y + 140, appear(t, 0.36, 0.18), 40)
    chain = [("盲审", 0.42), ("挂名", 0.58), ("签字", 0.74)]
    for i, (label, ts) in enumerate(chain):
        a = appear(t, ts, 0.18)
        x = 90 + i * 310
        yy = 700 + bob(t + ts, 6, 0.55)
        rounded(d, (x, yy, x + 280, yy + 200), 24, mix(BG, CARD, a))
        d.text((x + 140, yy + 70), label, font=font(36), fill=mix(CARD, MUTED, a), anchor="mm")
        d.text((x + 140, yy + 140), "堵死", font=font(32), fill=mix(CARD, RED, a), anchor="mm")
    return img


def draw_b_name_on(t: float) -> Image.Image:
    img = new_bg(t)
    d = ImageDraw.Draw(img)
    tag(d, t, "对 · 名字挂上")
    d.text((W // 2, 220 + bob(t, 6, 0.5)), "名字挂上才算数", font=font(48), fill=mix(BG, WHITE, appear(t, 0.02)), anchor="mm")
    a1 = appear(t, 0.12)
    y = 310 + bob(t, 6, 0.45)
    rounded(d, (90, y, 990, y + 360), 28, mix(BG, CARD, a1))
    d.text((540, y + 70), "专利 / 论文", font=font(32), fill=mix(CARD, MUTED, a1), anchor="mm")
    d.text((360, y + 180), "发明人", font=font(28), fill=mix(CARD, MUTED, a1), anchor="mm")
    name_a = appear(t, 0.40, 0.28)
    slot = mix(CARD, (18, 42, 36), name_a)
    rounded(d, (480, y + 130, 900, y + 250), 18, slot)
    d.text((690, y + 190), "名字已挂", font=font(40), fill=mix(CARD, MINT, name_a), anchor="mm")
    miss = appear(t, 0.85, 0.22)
    if miss > 0.04:
        yy = 740 + bob(t, 7, 0.7)
        rounded(d, (90, yy, 500, yy + 280), 26, mix(BG, (22, 40, 36), miss))
        d.text((295, yy + 80), "两件齐", font=font(32), fill=mix(CARD, MUTED, miss), anchor="mm")
        check_badge(d, 295, yy + 180, miss)
        rounded(d, (560, yy, 990, yy + 280), 26, mix(BG, (42, 24, 22), miss))
        d.text((775, yy + 80), "缺一件", font=font(32), fill=mix(CARD, MUTED, miss), anchor="mm")
        d.text((775, yy + 180), "停门口", font=font(44), fill=mix(CARD, RED, miss), anchor="mm")
    punch = appear(t, 1.20, 0.2)
    if punch > 0.04:
        y = 1080 + bob(t, 6, 0.8)
        rounded(d, (120, y, 960, y + 150), 26, mix(BG, (42, 24, 22), punch))
        d.text((W // 2, y + 75), "缺一件就不放行", font=font(44), fill=mix(BG, YELLOW, punch), anchor="mm")
    return img


BROLL_DRAW = {
    "not_top": draw_b_not_top,
    "three_cards": draw_b_three_cards,
    "stamp_pass": draw_b_stamp_pass,
    "window_block": draw_b_window_block,
    "name_on": draw_b_name_on,
}
...
def build_timeline(cues: list[tuple[float, float, str]], duration: float) -> dict:
    ...
    data = {
        ...
        "b_lead_s": LEAD,
        "video_type": "普通短视频",
        "source_note": "topics-batch4.md #85 / 备忘录第一节 3 毕业硬性底线 / 云端 NEW",
    }
...
def make_cover() -> Path:
    ...
    d.text((W // 2, 160), "毕业底线", font=font(52), fill=WHITE, anchor="mm")
    d.text((W // 2, 250), "三件套", font=font(72), fill=YELLOW, anchor="mm")
...
def write_docs(duration: float, staged: Path, data: dict) -> None:
    ...
    status = {
        ...
        "distinct_from": "成片/14-导师课题毕业就业技术栈自己建.mp4",
        "cloud_only": True,
        "windows_paths": False,
        "drive_upload": False,
        "not": "drama-pipeline / 仙侠连载 / C:D:G: / Drive 上传 / 成片14双线 / 15生态位 / 16切片 / 49精读 / 50绑定博士",
        "updated_at": now,
    }
...
| 号 | 成片 | 对照 14 | 新时长 | 草稿 |
|----|------|---------|--------|------|
| 85 | `{STAGED_NAME}` | 十四点一秒（双线自己建） | {zh_sec(duration)} | `.abroll-cloud/85/` |
...
def patch_delivery(duration: float) -> None:
    ...
    row = (
        ...
        f"topic 85 `topics-batch4.md` 第 85 条；Drive 只读备忘录第一节 3「毕业硬性底线」。云端 NEW，不拷成片 14 |"
    )
    ...
    if token in text:
        ...
        if marker in text:
            ...
            if not text.endswith("\n"):
                text += "\n"
            text += row + "\n"
    plus_row = f"| 85 | `{STAGED_NAME}` | {zh_sec(duration)} | 已核验 |"
    if "| 85 |" in text:
        rows = []
        for raw in text.splitlines():
            if raw.startswith("| 85 |"):
...
def qa(staged: Path, data: dict) -> dict:
    ...
    banned = ["下一期", "赵老师", "生态位", "主力博士", "就业技术栈", "十五十五七十"]
