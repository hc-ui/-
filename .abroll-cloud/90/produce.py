#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Topic 90：从课题抽可迁移能力。学习基线提取可迁移件。云端 NEW A-roll + B-roll。

普通短视频 / 知识口播。草稿只写 .abroll-cloud/90/，成品中转 成片/90-从课题抽可迁移能力.mp4。
禁止 C:\\ D:\\ G:\\，禁止 Drive 上传，不走 drama-pipeline。
目标四十到五十秒。不定格、不慢放注水。时长用中文「秒」。
不重做成片 14 双线，不重做成片 15 生态位。
"""
...
def draw_b_leave_take(t: float) -> Image.Image:
    img = new_bg(t)
    d = ImageDraw.Draw(img)
    tag(d, t, "对 · 先分清")
    d.text((W // 2, 236 + bob(t, 6, 0.5)), "留下场景 抽走方法", font=font(48), fill=mix(BG, WHITE, appear(t, 0.02)), anchor="mm")
    a1 = appear(t, 0.12)
    y = 340 + bob(t, 8, 0.5)
    rounded(d, (70, y, 500, y + 520), 32, mix(BG, (42, 28, 22), a1))
    d.text((285, y + 80), "留下", font=font(28), fill=mix(CARD, MUTED, a1), anchor="mm")
    d.text((285, y + 200), "场景", font=font(56), fill=mix(CARD, YELLOW, a1), anchor="mm")
    d.text((285, y + 300), "湿实验台", font=font(36), fill=mix(CARD, WHITE, a1), anchor="mm")
    d.text((285, y + 400), "领域名", font=font(32), fill=mix(CARD, MUTED, a1), anchor="mm")
    pull = appear(t, 0.28)
    xoff = int(lerp(0, 18, pull))
    rounded(d, (580 + xoff, y + bob(t + 0.15, 7, 0.45), 1010 + xoff, y + 520), 32, mix(BG, (18, 42, 36), pull))
    d.text((795 + xoff, y + 80), "抽走", font=font(28), fill=mix(CARD, MUTED, pull), anchor="mm")
    d.text((795 + xoff, y + 200), "方法", font=font(56), fill=mix(CARD, MINT, pull), anchor="mm")
    d.text((795 + xoff, y + 300), "闭环建模", font=font(36), fill=mix(CARD, WHITE, pull), anchor="mm")
    d.text((795 + xoff, y + 400), "可带走", font=font(32), fill=mix(CARD, MINT, pull), anchor="mm")
    if pull > 0.2:
        d.polygon([(520, y + 250), (560, y + 220), (560, y + 280)], fill=mix(CARD, MINT, pull))
    punch_bar(d, t, 1.00, "身份别一起背", mint=False)
    return img


def draw_b_sensor_code(t: float) -> Image.Image:
    img = new_bg(t)
    d = ImageDraw.Draw(img)
    tag(d, t, "例 · 采集")
    d.text((W // 2, 236 + bob(t, 6, 0.5)), "抽的是接口", font=font(52), fill=mix(BG, WHITE, appear(t, 0.02)), anchor="mm")
    a1 = appear(t, 0.12)
    y = 340 + bob(t, 8, 0.5)
    rounded(d, (70, y, 500, y + 480), 32, mix(BG, (42, 24, 22), a1))
    d.text((285, y + 90), "试剂名", font=font(32), fill=mix(CARD, MUTED, a1), anchor="mm")
    d.text((285, y + 210), "场景残留", font=font(40), fill=mix(CARD, RED, a1), anchor="mm")
    strike_line(d, (110, y + 180, 460, y + 240), appear(t, 0.40, 0.4), mix(CARD, RED, 1))
    draw_x(d, 285, y + 340, appear(t, 0.48, 0.2), 48)
    a2 = appear(t, 0.28)
    rounded(d, (580, y + bob(t + 0.2, 7, 0.5), 1010, y + 480), 32, mix(BG, (18, 42, 36), a2))
    d.text((795, y + 90), "传感器", font=font(32), fill=mix(CARD, MUTED, a2), anchor="mm")
    d.text((795, y + 210), "进代码", font=font(48), fill=mix(CARD, MINT, a2), anchor="mm")
    d.text((795, y + 300), "采集脚本", font=font(32), fill=mix(CARD, WHITE, a2), anchor="mm")
    check_badge(d, 795, y + 390, appear(t, 0.70, 0.18))
    punch_bar(d, t, 1.00, "不是试剂名")
    return img


def draw_b_resume(t: float) -> Image.Image:
    img = new_bg(t)
    d = ImageDraw.Draw(img)
    tag(d, t, "错 · 领域当身份")
    d.text((W // 2, 236 + bob(t, 6, 0.5)), "简历别写做过生医", font=font(46), fill=mix(BG, WHITE, appear(t, 0.02)), anchor="mm")
    a1 = appear(t, 0.12)
    y = 340 + bob(t, 7, 0.5)
    rounded(d, (110, y, 970, y + 260), 32, mix(BG, (42, 24, 22), a1))
    d.text((W // 2, y + 80), "我做过生医", font=font(48), fill=mix(CARD, RED, a1), anchor="mm")
    strike_line(d, (200, y + 50, 880, y + 120), appear(t, 0.36, 0.4), mix(CARD, RED, 1))
    d.text((W // 2, y + 180), "领域身份 带不走", font=font(30), fill=mix(CARD, MUTED, a1), anchor="mm")
    chips = [(0.40, 90, "会建回路", YELLOW), (0.55, 560, "会处理数据", MINT)]
    for ts, x, label, color in chips:
        a = appear(t, ts, 0.22)
        if a < 0.04:
            continue
        cy = 680 + bob(t + ts, 6, 0.6)
        rounded(d, (x, cy, x + 430, cy + 220), 28, mix(BG, CARD, a))
        d.text((x + 215, cy + 110), label, font=font(40), fill=mix(CARD, color, a), anchor="mm")
        check_badge(d, x + 360, cy + 50, appear(t, ts + 0.25, 0.18))
    punch_bar(d, t, 1.05, "写能带走的")
    return img


def draw_b_skill_list(t: float) -> Image.Image:
    img = new_bg(t)
    d = ImageDraw.Draw(img)
    tag(d, t, "对 · 清单")
    d.text((W // 2, 236 + bob(t, 6, 0.5)), "可迁移五件", font=font(54), fill=mix(BG, WHITE, appear(t, 0.02)), anchor="mm")
    items = [
        (0.10, 70, 340, "01", "处理"),
        (0.22, 560, 340, "02", "控制"),
        (0.34, 70, 560, "03", "建模"),
    ...
    for ts, x, y0, num, label in items:
        a = appear(t, ts, 0.20)
        if a < 0.04:
            continue
        y = y0 + bob(t + ts, 6, 0.55)
        w = 900 if num == "05" else 450
        rounded(d, (x, y, x + w, y + 180), 28, mix(BG, CARD, a))
        d.rounded_rectangle((x + 24, y + 56, x + 110, y + 124), 16, fill=mix(CARD, YELLOW if num in ("01", "03", "05") else MINT, a))
        d.text((x + 67, y + 90), num, font=font(28), fill=mix(YELLOW, INK, a), anchor="mm")
        d.text((x + w // 2 + 20, y + 90), label, font=font(48), fill=mix(CARD, WHITE, a), anchor="mm")
    punch_bar(d, t, 1.10, "先列再抽")
    return img


def draw_b_paper_skill(t: float) -> Image.Image:
    img = new_bg(t)
    d = ImageDraw.Draw(img)
    tag(d, t, "收 · 分开写")
    d.text((W // 2, 236 + bob(t, 6, 0.5)), "领域留论文", font=font(52), fill=mix(BG, WHITE, appear(t, 0.02)), anchor="mm")
    a1 = appear(t, 0.12)
    y = 340 + bob(t, 7, 0.5)
    rounded(d, (70, y, 500, y + 480), 32, mix(BG, CARD, a1))
    d.text((285, y + 80), "论文里", font=font(28), fill=mix(CARD, MUTED, a1), anchor="mm")
    d.text((285, y + 200), "领域名", font=font(48), fill=mix(CARD, YELLOW, a1), anchor="mm")
    d.text((285, y + 300), "可以留下", font=font(32), fill=mix(CARD, WHITE, a1), anchor="mm")
    a2 = appear(t, 0.28)
    rounded(d, (580, y + bob(t + 0.15, 6, 0.45), 1010, y + 480), 32, mix(BG, (18, 42, 36), a2))
    d.text((795, y + 80), "下一份工", font=font(28), fill=mix(CARD, MUTED, a2), anchor="mm")
    d.text((795, y + 200), "能力名", font=font(48), fill=mix(CARD, MINT, a2), anchor="mm")
    d.text((795, y + 300), "单独站", font=font(32), fill=mix(CARD, WHITE, a2), anchor="mm")
    check_badge(d, 795, y + 390, appear(t, 0.70, 0.18))
    punch_bar(d, t, 1.00, "课题是矿")
    return img


BROLL_DRAW = {
    "leave_take": draw_b_leave_take,
    "sensor_code": draw_b_sensor_code,
    "resume": draw_b_resume,
    "skill_list": draw_b_skill_list,
    "paper_skill": draw_b_paper_skill,
}
...
def build_timeline(cues: list[tuple[float, float, str]], duration: float) -> dict:
    ...
    data = {
        ...
        "b_lead_s": LEAD,
        "video_type": "普通短视频",
        "source_note": "topics-batch4.md #90 / 学习基线提取可迁移能力 / 云端 NEW",
    }
...
def make_cover() -> Path:
    ...
    d.text((W // 2, 160), "从课题抽", font=font(52), fill=WHITE, anchor="mm")
    d.text((W // 2, 250), "可迁移能力", font=font(64), fill=YELLOW, anchor="mm")
...
def write_docs(duration: float, staged: Path, data: dict) -> None:
    ...
    status = {
        ...
        "distinct_from": ["成片/14-导师课题毕业就业技术栈自己建.mp4", "成片/15-在生医实验室抢测控生态位.mp4"],
        "cloud_only": True,
        "windows_paths": False,
        "drive_upload": False,
        "not": "drama-pipeline / 仙侠连载 / C:D:G: / Drive 上传 / 成片14双线 / 成片15生态位",
        "updated_at": now,
    }
...
|----|------|---------|---------|--------|------|
| 90 | `{STAGED_NAME}` | 十四点一秒（双线） | 十五点三秒（生态位） | {zh_sec(duration)} | `.abroll-cloud/90/` |
...
def patch_delivery(duration: float) -> None:
    ...
    row = (
        ...
        f"topic 90 `topics-batch4.md` 第 90 条；Drive 只读学习基线提取可迁移能力。云端 NEW，不重做 14 / 15 |"
    )
    ...
    if token in text:
        ...
        if marker in text:
            ...
            if not text.endswith("\n"):
                text += "\n"
            text += row + "\n"
    plus_row = f"| 90 | `{STAGED_NAME}` | {zh_sec(duration)} | 已核验 |"
    if "| 90 |" in text:
        rows = []
        for raw in text.splitlines():
...
def qa(staged: Path, data: dict) -> dict:
    ...
    banned = ["两条线", "就业技术栈", "Python 学扎实", "五条路", "抢生态位", "测控底层", "包装成", "下一期", "赵老师"]
    ...
    report["ok"] = (
        ...
        and all(v["exists"] for v in report["keep_14_15"].values())
        and staged.name.startswith("90-")
    )
