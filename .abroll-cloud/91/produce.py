#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Topic 91：日志为什么出事才去翻。工厂 91 未进成片。云端 NEW A-roll + B-roll。

普通短视频 / 知识口播。草稿只写 .abroll-cloud/91/，成品中转 成片/91-日志为什么出事才去翻.mp4。
禁止 C:\\ D:\\ G:\\，禁止 Drive 上传，不走 drama-pipeline。
目标四十到五十秒。不定格、不慢放注水。时长用中文「秒」。
不重做成片 38 分不清改物理和AI画错。不拍片头纪律。
"""
...
def draw_b_unread(t: float) -> Image.Image:
    img = new_bg(t)
    d = ImageDraw.Draw(img)
    tag(d, t, "诊 · 看不懂")
    d.text((W // 2, 236 + bob(t, 6, 0.5)), "翻了也看不懂", font=font(52), fill=mix(BG, WHITE, appear(t, 0.02)), anchor="mm")
    a1 = appear(t, 0.12)
    y = 340 + bob(t, 8, 0.5)
    rounded(d, (90, y, 990, y + 260), 32, mix(BG, (42, 24, 22), a1))
    d.text((W // 2, y + 80), "出错了", font=font(56), fill=mix(CARD, RED, a1), anchor="mm")
    strike_line(d, (220, y + 50, 860, y + 120), appear(t, 0.40, 0.4), mix(CARD, RED, 1))
    d.text((W // 2, y + 180), "这句等于没记", font=font(32), fill=mix(CARD, MUTED, a1), anchor="mm")
    a2 = appear(t, 0.42)
    rounded(d, (90, y + 300, 990, y + 560), 32, mix(BG, (18, 42, 36), a2))
    d.text((W // 2, y + 430), "看不懂就不算记", font=font(48), fill=mix(CARD, MINT, a2), anchor="mm")
    punch_bar(d, t, 1.00, "出事才去翻", mint=False)
    return img


def draw_b_fields(t: float) -> Image.Image:
    img = new_bg(t)
    d = ImageDraw.Draw(img)
    tag(d, t, "01 三条字段")
    d.text((W // 2, 236 + bob(t, 6, 0.5)), "必看这三条", font=font(54), fill=mix(BG, WHITE, appear(t, 0.02)), anchor="mm")
    items = [
        (0.10, 90, 340, "01", "时间"),
        (0.26, 90, 560, "02", "用户"),
        (0.42, 90, 780, "03", "错误码"),
    ...
    for ts, x, y0, num, label in items:
        ...
        rounded(d, (x, y, 990, y + 180), 28, mix(BG, CARD, a))
        d.rounded_rectangle((x + 28, y + 52, x + 130, y + 128), 16, fill=mix(CARD, YELLOW if num != "02" else MINT, a))
        d.text((x + 79, y + 90), num, font=font(28), fill=mix(YELLOW, INK, a), anchor="mm")
        d.text((540, y + 90), label, font=font(52), fill=mix(CARD, WHITE, a), anchor="mm")
        check_badge(d, 900, y + 90, appear(t, ts + 0.22, 0.16))
    punch_bar(d, t, 1.10, "少一条就翻不出")
    return img


def draw_b_trace(t: float) -> Image.Image:
    img = new_bg(t)
    d = ImageDraw.Draw(img)
    tag(d, t, "02 追踪号")
    d.text((W // 2, 236 + bob(t, 6, 0.5)), "同一个号跟到底", font=font(50), fill=mix(BG, WHITE, appear(t, 0.02)), anchor="mm")
    nodes = [(0.12, 90, "入口"), (0.30, 390, "服务"), (0.48, 690, "下游")]
    y = 420 + bob(t, 7, 0.5)
    for i, (ts, x, label) in enumerate(nodes):
        a = appear(t, ts, 0.22)
        if a < 0.04:
            continue
        rounded(d, (x, y, x + 260, y + 280), 28, mix(BG, CARD, a))
        d.text((x + 130, y + 80), label, font=font(36), fill=mix(CARD, MUTED, a), anchor="mm")
        d.text((x + 130, y + 170), "T-1842", font=font(40), fill=mix(CARD, MINT, a), anchor="mm")
        if i and appear(t, ts, 0.18) > 0.3:
            d.polygon([(x - 28, y + 130), (x - 8, y + 110), (x - 8, y + 150)], fill=mix(CARD, MINT, a))
    punch_bar(d, t, 1.05, "只认这一个号")
    return img
...
def draw_b_mismatch(t: float) -> Image.Image:
    ...
    d = ImageDraw.Draw(img)
    tag(d, t, "错 · 各写各的")
    d.text((W // 2, 236 + bob(t, 6, 0.5)), "各写各的号", font=font(52), fill=mix(BG, WHITE, appear(t, 0.02)), anchor="mm")
    a1 = appear(t, 0.12)
    y = 340 + bob(t, 8, 0.5)
    rounded(d, (70, y, 500, y + 480), 32, mix(BG, (42, 24, 22), a1))
    d.text((285, y + 90), "入口", font=font(28), fill=mix(CARD, MUTED, a1), anchor="mm")
    d.text((285, y + 200), "A-991", font=font(48), fill=mix(CARD, RED, a1), anchor="mm")
    d.text((285, y + 300), "下游", font=font(28), fill=mix(CARD, MUTED, a1), anchor="mm")
    d.text((285, y + 390), "B-027", font=font(48), fill=mix(CARD, RED, a1), anchor="mm")
    draw_x(d, 285, y + 140, appear(t, 0.50, 0.2), 36)
    a2 = appear(t, 0.32)
    rounded(d, (580, y + bob(t + 0.15, 6, 0.45), 1010, y + 480), 32, mix(BG, (18, 42, 36), a2))
    d.text((795, y + 120), "没号", font=font(36), fill=mix(CARD, MUTED, a2), anchor="mm")
    d.text((795, y + 240), "不准进", font=font(52), fill=mix(CARD, MINT, a2), anchor="mm")
    d.text((795, y + 360), "卡住不发", font=font(32), fill=mix(CARD, WHITE, a2), anchor="mm")
    punch_bar(d, t, 1.00, "对不上只能猜", mint=False)
    return img


def draw_b_gate(t: float) -> Image.Image:
    img = new_bg(t)
    d = ImageDraw.Draw(img)
    tag(d, t, "03 没号不准进")
    d.text((W // 2, 236 + bob(t, 6, 0.5)), "不准进生产", font=font(54), fill=mix(BG, WHITE, appear(t, 0.02)), anchor="mm")
    rows = [
        (0.12, "有追踪号", "才能发布", True),
        (0.32, "没有号", "卡住不发", True),
        (0.52, "进了生产", "出事只能猜", False),
    ]
    for i, (ts, left, right, ok) in enumerate(rows):
        a = appear(t, ts, 0.20)
        if a < 0.04:
            continue
        y = 360 + i * 200 + bob(t + ts, 5, 0.55)
        rounded(d, (80, y, 1000, y + 170), 28, mix(BG, CARD, a))
        d.text((280, y + 85), left, font=font(36), fill=mix(CARD, WHITE, a), anchor="mm")
        d.text((760, y + 85), right, font=font(36), fill=mix(CARD, MINT if ok else RED, a), anchor="mm")
        if ok:
            check_badge(d, 120, y + 85, appear(t, ts + 0.18, 0.16))
        else:
            draw_x(d, 120, y + 85, appear(t, ts + 0.18, 0.16), 28)
    punch_bar(d, t, 1.15, "出事才翻得出来")
    return img


BROLL_DRAW = {
    "unread": draw_b_unread,
    "fields": draw_b_fields,
    "trace": draw_b_trace,
...
def build_timeline(cues: list[tuple[float, float, str]], duration: float) -> dict:
    ...
    data = {
        ...
        "b_lead_s": LEAD,
        "video_type": "普通短视频",
        "source_note": "topics-batch4.md #91 / 工厂 91_日志为什么出事才去翻 / 云端 NEW / 不重做 38",
    }
...
def make_cover() -> Path:
    ...
    d.text((W // 2, 160), "日志为什么", font=font(52), fill=WHITE, anchor="mm")
    d.text((W // 2, 250), "出事才去翻", font=font(64), fill=YELLOW, anchor="mm")
...
def write_docs(duration: float, staged: Path, data: dict) -> None:
    ...
    status = {
        ...
        "distinct_from": ["成片/38-分不清改物理和AI画错.mp4", "成片/69-值班手册是空的.mp4"],
        "cloud_only": True,
        "windows_paths": False,
        "drive_upload": False,
        "not": "drama-pipeline / 仙侠连载 / C:D:G: / Drive 上传 / 成片38改物理画错 / 片头纪律",
        "updated_at": now,
    }
    (ROOT / "项目状态.json").write_text(json.dumps(status, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")

    note = f"""# 91 · 日志为什么出事才去翻
...
|----|------|---------|----------|--------|------|
| 91 | `{STAGED_NAME}` | 十九点九秒（改物理/画错） | 三十一点九秒 | {zh_sec(duration)} | `.abroll-cloud/91/` |
...
def patch_delivery(duration: float) -> None:
    ...
    row = (
        ...
        f"topic 91 `topics-batch4.md` 第 91 条；Drive 只读工厂 `91_日志为什么出事才去翻`。云端 NEW，不重做 38 |"
    )
    ...
    if token in text:
        ...
        if marker in text:
            ...
            if not text.endswith("\n"):
                text += "\n"
            text += row + "\n"
    plus_row = f"| 91 | `{STAGED_NAME}` | {zh_sec(duration)} | 已核验 |"
    if "| 91 |" in text:
        rows = []
        for raw in text.splitlines():
...
def qa(staged: Path, data: dict) -> dict:
    ...
    banned = ["改物理", "画错", "片头", "雨滴", "穿帮", "抽卡", "下一期", "赵老师", "大促", "学完立刻"]
    report["script_guard"] = {w: (w not in vo) for w in banned}
    keep = {
        "69": Path("/workspace/成片/69-值班手册是空的.mp4"),
        "84": Path("/workspace/成片/84-接口变更口头说一声.mp4"),
    }
    report["keep_neighbors"] = {
        key: {"exists": path.exists(), "bytes": path.stat().st_size if path.exists() else 0}
        for key, path in keep.items()
    }
    report["not_38_title"] = "分不清" not in staged.name and "片头纪律" not in staged.name
    report["ok"] = (
        ...
        and staged.name.startswith("91-")
        and report["not_38_title"]
    )
