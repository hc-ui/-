#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Topic 93：研一结束前用项目判断方向。学习基线用真实项目判断方向。云端 NEW A-roll + B-roll。

普通短视频 / 知识口播。草稿只写 .abroll-cloud/93/，成品中转 成片/93-研一结束前用项目判断方向.mp4。
禁止 C:\\ D:\\ G:\\，禁止 Drive 上传，不走 drama-pipeline。
目标四十到五十秒。不定格、不慢放注水。时长用中文「秒」。
不重做成片 14 双线，不重做成片 15 生态位，不重做成片 51 第二技能树，不重做成片 90 可迁移。
"""
...
NAME = "研一结束前用项目判断方向"
STAGED_NAME = "93-研一结束前用项目判断方向.mp4"
...
def punch_bar(draw: ImageDraw.ImageDraw, t: float, start: float, text: str, mint: bool = True) -> None:
    a = appear(t, start, 0.22)
    if a <= 0.04:
        return
    y = 1040 + bob(t, 7, 0.8)
    fill = mix(BG, (18, 42, 36) if mint else (42, 32, 16), a)
    ink = mix(BG, MINT if mint else YELLOW, a)
    rounded(draw, (120, y, 960, y + 170), 28, fill)
    draw.text((W // 2, y + 85), text, font=font(48), fill=ink, anchor="mm")


def draw_b_three_lanes(t: float) -> Image.Image:
    img = new_bg(t)
    d = ImageDraw.Draw(img)
    tag(d, t, "判 · 三条")
    d.text((W // 2, 236 + bob(t, 6, 0.5)), "三条候选", font=font(54), fill=mix(BG, WHITE, appear(t, 0.02)), anchor="mm")
    items = [
        (0.10, 70, 340, "01", "机器人算法", YELLOW),
        (0.28, 560, 340, "02", "具身智能", MINT),
        (0.46, 70, 620, "03", "智能体工程", YELLOW),
    ]
    for ts, x, y0, num, label, color in items:
        a = appear(t, ts, 0.22)
        if a < 0.04:
            continue
        y = y0 + bob(t + ts, 6, 0.55)
        w = 900 if num == "03" else 450
        rounded(d, (x, y, x + w, y + 220), 28, mix(BG, CARD, a))
        d.rounded_rectangle((x + 24, y + 70, x + 110, y + 150), 16, fill=mix(CARD, color, a))
        d.text((x + 67, y + 110), num, font=font(28), fill=mix(color, INK, a), anchor="mm")
        d.text((x + w // 2 + 24, y + 110), label, font=font(44), fill=mix(CARD, WHITE, a), anchor="mm")
    punch_bar(d, t, 1.05, "用项目来判")
    return img


def draw_b_imagine_empty(t: float) -> Image.Image:
    img = new_bg(t)
    d = ImageDraw.Draw(img)
    tag(d, t, "错 · 空想")
    d.text((W // 2, 236 + bob(t, 6, 0.5)), "看完还是空", font=font(52), fill=mix(BG, WHITE, appear(t, 0.02)), anchor="mm")
    a1 = appear(t, 0.12)
    y = 340 + bob(t, 8, 0.5)
    rounded(d, (70, y, 500, y + 520), 32, mix(BG, CARD, a1))
    d.text((285, y + 70), "课 / 视频", font=font(28), fill=mix(CARD, MUTED, a1), anchor="mm")
    d.text((285, y + 200), "刷完了", font=font(48), fill=mix(CARD, YELLOW, a1), anchor="mm")
    d.text((285, y + 300), "收藏夹打勾", font=font(32), fill=mix(CARD, WHITE, a1), anchor="mm")
    check_badge(d, 285, y + 410, appear(t, 0.40, 0.18))
    a2 = appear(t, 0.28)
    rounded(d, (580, y + bob(t + 0.15, 7, 0.45), 1010, y + 520), 32, mix(BG, (42, 24, 22), a2))
    d.text((795, y + 70), "方向格", font=font(28), fill=mix(CARD, MUTED, a2), anchor="mm")
    d.text((795, y + 200), "还是空", font=font(48), fill=mix(CARD, RED, a2), anchor="mm")
    strike_line(d, (620, y + 170, 970, y + 230), appear(t, 0.50, 0.4), mix(CARD, RED, 1))
    draw_x(d, 795, y + 380, appear(t, 0.62, 0.2), 48)
    punch_bar(d, t, 1.00, "想象定不了", mint=False)
    return img


def draw_b_two_projects(t: float) -> Image.Image:
    img = new_bg(t)
    d = ImageDraw.Draw(img)
    tag(d, t, "对 · 先做")
    d.text((W // 2, 236 + bob(t, 6, 0.5)), "两个小件", font=font(54), fill=mix(BG, WHITE, appear(t, 0.02)), anchor="mm")
    a1 = appear(t, 0.12)
    y = 340 + bob(t, 8, 0.5)
    rounded(d, (70, y, 500, y + 520), 32, mix(BG, (18, 42, 36), a1))
    d.text((285, y + 80), "智能体", font=font(28), fill=mix(CARD, MUTED, a1), anchor="mm")
    d.text((285, y + 200), "小件能跑", font=font(46), fill=mix(CARD, MINT, a1), anchor="mm")
    d.text((285, y + 300), "先交一件", font=font(32), fill=mix(CARD, WHITE, a1), anchor="mm")
    check_badge(d, 285, y + 410, appear(t, 0.48, 0.18))
    a2 = appear(t, 0.30)
    rounded(d, (580, y + bob(t + 0.2, 7, 0.5), 1010, y + 520), 32, mix(BG, (18, 42, 36), a2))
    d.text((795, y + 80), "机器人", font=font(28), fill=mix(CARD, MUTED, a2), anchor="mm")
    d.text((795, y + 200), "小件能动", font=font(46), fill=mix(CARD, YELLOW, a2), anchor="mm")
    d.text((795, y + 300), "再交一件", font=font(32), fill=mix(CARD, WHITE, a2), anchor="mm")
    check_badge(d, 795, y + 410, appear(t, 0.70, 0.18))
    punch_bar(d, t, 1.05, "两条都试过")
    return img


def draw_b_want_again(t: float) -> Image.Image:
    img = new_bg(t)
    d = ImageDraw.Draw(img)
    tag(d, t, "判 · 手感")
    d.text((W // 2, 236 + bob(t, 6, 0.5)), "还想再做", font=font(54), fill=mix(BG, WHITE, appear(t, 0.02)), anchor="mm")
    lanes = [
        (0.12, 340, "机器人算法", False),
        (0.28, 560, "具身智能", True),
        (0.44, 780, "智能体工程", False),
    ]
    for ts, y0, label, pick in lanes:
        a = appear(t, ts, 0.20)
        if a < 0.04:
            continue
        y = y0 + bob(t + ts, 5, 0.55)
        fill = (18, 42, 36) if pick else CARD
        rounded(d, (90, y, 990, y + 180), 28, mix(BG, fill, a))
        d.text((W // 2, y + 90), label, font=font(44), fill=mix(CARD, MINT if pick else WHITE, a), anchor="mm")
        if pick:
            check_badge(d, 930, y + 90, appear(t, ts + 0.22, 0.18))
        else:
            draw_x(d, 930, y + 90, appear(t, ts + 0.18, 0.16), 22)
    punch_bar(d, t, 1.10, "手熟的才算")
    return img


def draw_b_scale(t: float) -> Image.Image:
    img = new_bg(t)
    d = ImageDraw.Draw(img)
    tag(d, t, "收 · 窗口")
    d.text((W // 2, 236 + bob(t, 6, 0.5)), "研一结束前", font=font(52), fill=mix(BG, WHITE, appear(t, 0.02)), anchor="mm")
    a1 = appear(t, 0.12)
    y = 340 + bob(t, 7, 0.5)
    rounded(d, (70, y, 500, y + 480), 32, mix(BG, (18, 42, 36), a1))
    d.text((285, y + 80), "项目", font=font(28), fill=mix(CARD, MUTED, a1), anchor="mm")
    d.text((285, y + 200), "是秤", font=font(56), fill=mix(CARD, MINT, a1), anchor="mm")
    d.text((285, y + 300), "亲手做完", font=font(32), fill=mix(CARD, WHITE, a1), anchor="mm")
    check_badge(d, 285, y + 390, appear(t, 0.55, 0.18))
    a2 = appear(t, 0.28)
    rounded(d, (580, y + bob(t + 0.15, 6, 0.45), 1010, y + 480), 32, mix(BG, (42, 24, 22), a2))
    d.text((795, y + 80), "海报", font=font(28), fill=mix(CARD, MUTED, a2), anchor="mm")
    d.text((795, y + 200), "不是秤", font=font(48), fill=mix(CARD, RED, a2), anchor="mm")
    strike_line(d, (620, y + 170, 970, y + 230), appear(t, 0.48, 0.4), mix(CARD, RED, 1))
    draw_x(d, 795, y + 360, appear(t, 0.62, 0.2), 44)
    punch_bar(d, t, 1.00, "窗口就在这")
    return img


BROLL_DRAW = {
    "three_lanes": draw_b_three_lanes,
    "imagine_empty": draw_b_imagine_empty,
    "two_projects": draw_b_two_projects,
    "want_again": draw_b_want_again,
    "scale": draw_b_scale,
}
...
def build_timeline(cues: list[tuple[float, float, str]], duration: float) -> dict:
    ...
    data = {
        ...
        "b_lead_s": LEAD,
        "video_type": "普通短视频",
        "factory": "topics-batch4 #93",
        "source_note": "topics-batch4.md #93 / 学习基线研一结束前用项目判断方向 / 云端 NEW",
    }
...
def make_cover() -> Path:
    ...
    d.text((W // 2, 160), "研一结束前", font=font(52), fill=WHITE, anchor="mm")
    d.text((W // 2, 250), "用项目判断方向", font=font(58), fill=YELLOW, anchor="mm")
...
def write_docs(duration: float, staged: Path, data: dict) -> None:
    ...
    status = {
        "schema_version": 1,
        "project_name": "93_研一结束前用项目判断方向",
        "video_type": "普通短视频",
        "episode": 93,
        "title": NAME,
        "source_note": "topics-batch4.md #93 / 学习基线 2026-08-24 研一结束前用项目判断方向 / 云端 NEW",
        "slug": "93_研一结束前用项目判断方向",
        "duration_zh": zh_sec(duration),
        "voice": VOICE,
        "duration": round(duration, 3),
        "size": [W, H],
        ...
        "distinct_from": [
            ...
            "成片/51-Agent是第二技能树不是主线.mp4",
            "成片/90-从课题抽可迁移能力.mp4",
        ],
        "cloud_only": True,
        "windows_paths": False,
        "drive_upload": False,
        "not": "drama-pipeline / 仙侠连载 / C:D:G: / Drive 上传 / 成片14双线 / 15生态位 / 51第二技能树 / 90可迁移",
        "updated_at": now,
    }
    (ROOT / "项目状态.json").write_text(json.dumps(status, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")

    note = f"""# 93 · 研一结束前用项目判断方向
...
|----|------|---------|---------|---------|--------|------|
| 93 | `{STAGED_NAME}` | 十四点一秒（双线） | 十二点三秒（第二技能树） | 五十点零秒（可迁移） | {zh_sec(duration)} | `.abroll-cloud/93/` |
...
def patch_delivery(duration: float) -> None:
    ...
    row = (
        ...
        f"topic 93 `topics-batch4.md` 第 93 条；Drive 只读 `学习与职业规划基线.md`"
        f"（2026-08-24：研一结束前用真实项目判断方向）。云端 NEW，不重做 14 / 15 / 51 / 90 |"
    )
    ...
    if token in text:
        ...
        if marker in text:
            ...
            if not text.endswith("\n"):
                text += "\n"
            text += row + "\n"
    plus_row = f"| 93 | `{STAGED_NAME}` | {zh_sec(duration)} | 已核验 |"
    if "| 93 |" in text:
        rows = []
        for raw in text.splitlines():
            if raw.startswith("| 93 |"):
...
def qa(staged: Path, data: dict) -> dict:
    ...
    report = {
        ...
        "distinct_from_14_15_51_90": True,
        "video": {
    ...
    banned = [
        "两条线", "就业技术栈", "Python 学扎实", "五条路", "抢生态位",
        "测控底层", "包装成", "下一期", "赵老师", "第二技能树", "可迁移",
    ]
    report["script_guard"] = {w: (w not in vo) for w in banned}
    keep = {
        "14": Path("/workspace/成片/14-导师课题毕业就业技术栈自己建.mp4"),
        "15": Path("/workspace/成片/15-在生医实验室抢测控生态位.mp4"),
        "51": Path("/workspace/成片/51-Agent是第二技能树不是主线.mp4"),
        "90": Path("/workspace/成片/90-从课题抽可迁移能力.mp4"),
    }
    report["keep_neighbors"] = {
    ...
    report["ok"] = (
        ...
        and all(v["exists"] for v in report["keep_neighbors"].values())
        and staged.name.startswith("93-")
    )
