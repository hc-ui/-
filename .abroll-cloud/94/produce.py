#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Topic 94：别在标题里堆感叹号。工厂 338。云端 NEW A-roll + B-roll。

普通短视频 / 知识口播。草稿只写 .abroll-cloud/94/，成品中转 成片/94-别在标题里堆感叹号.mp4。
禁止 C:\\ D:\\ G:\\，禁止 Drive 上传，不走 drama-pipeline。
目标四十到五十秒。不定格、不慢放注水。时长用中文「秒」。
不讲成片 52 反转剧透、不拷工厂十一秒短切。
"""
...
def draw_bang(draw: ImageDraw.ImageDraw, cx: int, cy: int, a: float, size: int = 96, color=RED) -> None:
    if a <= 0.04:
        return
    col = mix(CARD, color, a)
    draw.rectangle((cx - size // 7, cy - size, cx + size // 7, cy + size // 3), fill=col)
    draw.ellipse((cx - size // 6, cy + size // 2, cx + size // 6, cy + size // 2 + size // 3), fill=col)


def draw_meter(draw: ImageDraw.ImageDraw, x0: int, y0: int, w: int, h: int, fill: float, a: float) -> None:
    if a <= 0.04:
        return
    rounded(draw, (x0, y0, x0 + w, y0 + h), 18, mix(BG, (32, 24, 24), a))
    inner = max(8, int((w - 16) * max(0.06, min(1.0, fill))))
    bar_col = mix((80, 28, 28), MINT, fill)
    rounded(draw, (x0 + 8, y0 + 8, x0 + 8 + inner, y0 + h - 8), 12, mix(BG, bar_col, a))


def draw_b_bang_drop(t: float) -> Image.Image:
    img = new_bg(t)
    d = ImageDraw.Draw(img)
    tag(d, t, "错 · 堆感叹")
    d.text((W // 2, 220 + bob(t, 6, 0.5)), "三个感叹号叠上去", font=font(48), fill=mix(BG, WHITE, appear(t, 0.02)), anchor="mm")
    a1 = appear(t, 0.10)
    y = 320 + bob(t, 7, 0.45)
    rounded(d, (90, y, 990, y + 280), 32, mix(BG, CARD, a1))
    d.text((W // 2, y + 88), "搞定了", font=font(64), fill=mix(CARD, WHITE, a1), anchor="mm")
    for i, delay in enumerate((0.28, 0.48, 0.68)):
        drop = appear(t, delay, 0.22)
        if drop <= 0.04:
            continue
        bx = 390 + i * 150
        by = int(lerp(y - 80, y + 200, drop))
        draw_bang(d, bx, by, drop, 78, YELLOW if i < 2 else RED)
    meter_a = appear(t, 0.40)
    fill = max(0.12, 1.0 - appear(t, 0.55, 0.9) * 0.78)
    d.text((W // 2, 700), "可信度", font=font(32), fill=mix(BG, MUTED, meter_a), anchor="mm")
    draw_meter(d, 140, 740, 800, 72, fill, meter_a)
    d.text((W // 2, 880), "当场掉一截", font=font(44), fill=mix(BG, RED, appear(t, 0.70)), anchor="mm")
    punch = appear(t, 1.10, 0.22)
    if punch > 0.04:
        yy = 1000 + bob(t, 7, 0.8)
        rounded(d, (120, yy, 960, yy + 170), 28, mix(BG, (42, 24, 22), punch))
        d.text((W // 2, yy + 85), "像在喊你买", font=font(52), fill=mix(BG, YELLOW, punch), anchor="mm")
    return img


def draw_b_compare_cards(t: float) -> Image.Image:
    img = new_bg(t)
    d = ImageDraw.Draw(img)
    tag(d, t, "对照 · 对错卡")
    d.text((W // 2, 220 + bob(t, 6, 0.5)), "促销对笔记", font=font(52), fill=mix(BG, WHITE, appear(t, 0.02)), anchor="mm")
    a1 = appear(t, 0.12)
    y = 330 + bob(t, 8, 0.5)
    rounded(d, (50, y, 520, y + 620), 32, mix(BG, (42, 24, 22), a1))
    d.text((285, y + 70), "错的写法", font=font(30), fill=mix(CARD, MUTED, a1), anchor="mm")
    d.text((285, y + 200), "搞定了", font=font(48), fill=mix(CARD, WHITE, a1), anchor="mm")
    for i in range(3):
        draw_bang(d, 200 + i * 90, y + 340, appear(t, 0.28 + i * 0.08), 70, RED)
    d.text((285, y + 520), "三个感叹号", font=font(32), fill=mix(CARD, RED, a1), anchor="mm")
    draw_x(d, 285, y + 580, appear(t, 0.70, 0.2), 36)
    a2 = appear(t, 0.28)
    rounded(d, (560, y + bob(t + 0.15, 6, 0.45), 1030, y + 620), 32, mix(BG, (18, 40, 36), a2))
    d.text((795, y + 70), "对的写法", font=font(30), fill=mix(CARD, MUTED, a2), anchor="mm")
    d.text((795, y + 240), "搞定了", font=font(52), fill=mix(CARD, WHITE, a2), anchor="mm")
    d.text((795, y + 340), "。", font=font(96), fill=mix(CARD, MINT, a2), anchor="mm")
    d.text((795, y + 470), "句号收住", font=font(36), fill=mix(CARD, MINT, a2), anchor="mm")
    check_badge(d, 795, y + 560, appear(t, 0.80, 0.18))
    return img


def draw_b_hook_vs_punct(t: float) -> Image.Image:
    img = new_bg(t)
    d = ImageDraw.Draw(img)
    tag(d, t, "错 · 标点当钩")
    d.text((W // 2, 220 + bob(t, 6, 0.5)), "感叹号不是钩子", font=font(48), fill=mix(BG, WHITE, appear(t, 0.02)), anchor="mm")
    a1 = appear(t, 0.14)
    y = 340 + bob(t, 7, 0.5)
    rounded(d, (70, y, 500, y + 520), 32, mix(BG, (42, 24, 22), a1))
    d.text((285, y + 80), "标点", font=font(30), fill=mix(CARD, MUTED, a1), anchor="mm")
    draw_bang(d, 285, y + 230, a1, 120, RED)
    draw_x(d, 285, y + 400, appear(t, 0.50, 0.2), 52)
    a2 = appear(t, 0.28)
    rounded(d, (580, y + bob(t + 0.12, 6, 0.45), 1010, y + 520), 32, mix(BG, (18, 40, 36), a2))
    d.text((795, y + 80), "事实", font=font(30), fill=mix(CARD, MUTED, a2), anchor="mm")
    d.text((795, y + 230), "搞定了", font=font(52), fill=mix(CARD, WHITE, a2), anchor="mm")
    d.text((795, y + 320), "只留这句", font=font(36), fill=mix(CARD, MINT, a2), anchor="mm")
    check_badge(d, 795, y + 420, appear(t, 0.70, 0.18))
    punch = appear(t, 1.05, 0.22)
    if punch > 0.04:
        yy = 920 + bob(t, 7, 0.8)
        rounded(d, (120, yy, 960, yy + 170), 28, mix(BG, (18, 42, 36), punch))
        d.text((W // 2, yy + 85), "钩子靠事实", font=font(52), fill=mix(BG, MINT, punch), anchor="mm")
    return img


def draw_b_count_delete(t: float) -> Image.Image:
    img = new_bg(t)
    d = ImageDraw.Draw(img)
    tag(d, t, "收 · 超过就删")
    d.text((W // 2, 220 + bob(t, 6, 0.5)), "写完先数感叹号", font=font(48), fill=mix(BG, WHITE, appear(t, 0.02)), anchor="mm")
    a1 = appear(t, 0.12)
    y = 330 + bob(t, 7, 0.45)
    rounded(d, (90, y, 990, y + 420), 32, mix(BG, CARD, a1))
    d.text((W // 2, y + 80), "搞定了", font=font(52), fill=mix(CARD, WHITE, a1), anchor="mm")
    kill = appear(t, 0.70, 0.35)
    for i, label in enumerate(("1", "2", "3")):
        ax = 300 + i * 240
        ay = y + 230
        draw_bang(d, ax, ay, appear(t, 0.22 + i * 0.12), 88, YELLOW)
        d.text((ax, ay + 120), label, font=font(28), fill=mix(CARD, MUTED, a1), anchor="mm")
        if i > 0:
            draw_x(d, ax, ay, kill, 40)
            strike_line(d, (ax - 70, ay - 40, ax + 70, ay + 40), kill, mix(CARD, RED, 1))
    punch = appear(t, 1.05, 0.22)
    if punch > 0.04:
        yy = 820 + bob(t, 7, 0.8)
        rounded(d, (90, yy, 990, yy + 280), 28, mix(BG, (18, 42, 36), punch))
        d.text((W // 2, yy + 90), "超过一个，就全删", font=font(44), fill=mix(BG, MINT, punch), anchor="mm")
        d.text((W // 2, yy + 190), "标点救不了弱标题", font=font(36), fill=mix(BG, YELLOW, punch), anchor="mm")
    return img


BROLL_DRAW = {
    "bang_drop": draw_b_bang_drop,
    "compare_cards": draw_b_compare_cards,
    "hook_vs_punct": draw_b_hook_vs_punct,
    "count_delete": draw_b_count_delete,
}
...
def build_timeline(cues: list[tuple[float, float, str]], duration: float) -> dict:
    ...
    data = {
        ...
        "b_lead_s": LEAD,
        "video_type": "普通短视频",
        "source_note": "topics-batch4.md #94 / 工厂 338_别在标题里堆感叹号 / 云端 NEW",
    }
...
def make_cover() -> Path:
    ...
    d.text((W // 2, 160), "别在标题里", font=font(52), fill=WHITE, anchor="mm")
    d.text((W // 2, 250), "堆感叹号", font=font(72), fill=YELLOW, anchor="mm")
...
def write_docs(duration: float, staged: Path, data: dict) -> None:
    ...
    status = {
        ...
        "distinct_from": "成片/52-别把反转写进标题.mp4",
        "cloud_only": True,
        "windows_paths": False,
        "drive_upload": False,
        "not": "drama-pipeline / 仙侠连载 / C:D:G: / Drive 上传 / 成片52反转剧透 / 工厂338短切拷贝",
        "updated_at": now,
    }
...
| 号 | 成片 | 对照 52 | 新时长 | 草稿 |
|----|------|---------|--------|------|
| 94 | `{STAGED_NAME}` | 十二点八秒（反转剧透） | {zh_sec(duration)} | `.abroll-cloud/94/` |
...
def patch_delivery(duration: float) -> None:
    ...
    row = (
        ...
        f"topic 94 `topics-batch4.md` 第 94 条；Drive 只读工厂 `338_别在标题里堆感叹号`（工厂短切约十一秒，未进成片）。云端 NEW，不拷工厂成片，不重拍成片 52 |"
    )
    ...
    if token in text:
        ...
        if marker in text:
            ...
            if not text.endswith("\n"):
                text += "\n"
            text += row + "\n"
    plus_row = f"| 94 | `{STAGED_NAME}` | {zh_sec(duration)} | 已核验 |"
    if "| 94 |" in text:
        rows = []
        for raw in text.splitlines():
